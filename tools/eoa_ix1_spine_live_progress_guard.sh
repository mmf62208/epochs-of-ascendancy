...
if [[ ! -x "${ROOT}/tools/eoa_play_f5_smoke_auto_begin.sh" ]]; then
	echo "eoa_ix1_spine_live_progress_guard: missing tools/eoa_play_f5_smoke_auto_begin.sh" >&2
	exit 1
fi
if [[ ! -x "${ROOT}/tools/run_godot.sh" ]]; then
	echo "eoa_ix1_spine_live_progress_guard: missing tools/run_godot.sh" >&2
	exit 1
fi

export EOA_SMOKE_SPINE_LIVE_PROGRESS=1
export EOA_SMOKE_FRAME_GUARD=1
RSS_LIMIT_MB="${EOA_LIVE_PROGRESS_RSS_LIMIT_MB:-3072}"
LOG="${EOA_LIVE_PROGRESS_LOG:-/tmp/eoa-ix1-spine-live-progress.log}"
RSSLOG="${EOA_LIVE_PROGRESS_RSS_LOG:-/tmp/eoa-ix1-spine-live-progress-rss.log}"
rm -f "$LOG" "$RSSLOG"
...
if [[ -z "${DISPLAY:-}" ]]; then
	if ! command -v xvfb-run >/dev/null 2>&1; then
		echo "eoa_ix1_spine_live_progress_guard: no DISPLAY and no xvfb-run" >&2
		exit 1
	fi
...
(
	for _i in $(seq 1 900); do
		...
		if [[ -n "${GODOT_PID:-}" && -r "/proc/${GODOT_PID}/status" ]]; then
			...
			if grep -qE "EOA_SMOKE_SPINE_PROGRESS who=MapRenderer.button|EOA_SMOKE_FRAME_GUARD who=TestRunner.mouse_press|EOA_SMOKE_SPINE_LIVE_PROGRESS who=TestRunner.arm" "$LOG" 2>/dev/null; then
...
echo "==== Godot (EOA_SMOKE_SPINE_LIVE_PROGRESS) ===="
grep -E "EOA_SMOKE_SPINE_LIVE_PROGRESS|EOA_SMOKE_SPINE_START|EOA_SMOKE_SPINE_PROGRESS|EOA_SMOKE_SPINE_STATE|EOA_SMOKE_SPINE_COMPLETE|EOA_SMOKE_FRAME_GUARD who=TestRunner.mouse_press|RESULT=" "$LOG" | tail -n 120 || tail -n 60 "$LOG"
SIDECAR_MAX=0
SIDECAR_LAST=0
if [[ -f "$RSSLOG" ]]; then
	...
	echo "eoa_ix1_spine_live_progress_guard: sidecar_max_rss_mb=${SIDECAR_MAX} sidecar_last_rss_mb=${SIDECAR_LAST}"
fi
...
PROG_OK=0
if grep -qE "EOA_SMOKE_SPINE_LIVE_PROGRESS who=TestRunner.tick days=[5-9][0-9]* pct=[1-9]" "$LOG" \
	|| grep -qE "EOA_SMOKE_SPINE_PROGRESS who=IDM.advance_daily pid=710417 pct=[1-9]" "$LOG"; then
	PROG_OK=1
fi
COMPLETE_OK=0
if grep -qE "EOA_SMOKE_SPINE_LIVE_PROGRESS RESULT=PASS" "$LOG" \
	|| grep -qE "EOA_SMOKE_SPINE_COMPLETE" "$LOG"; then
	COMPLETE_OK=1
fi
GODOT_FAIL=0
if grep -qE "EOA_SMOKE_SPINE_LIVE_PROGRESS RESULT=FAIL" "$LOG"; then
	GODOT_FAIL=1
fi
RSS_OK=0
...
if [[ "$GODOT_FAIL" -eq 0 && "$PRESS_OK" -eq 1 && "$PROG_OK" -eq 1 && "$COMPLETE_OK" -eq 1 && "$RSS_OK" -eq 1 ]]; then
	echo "eoa_ix1_spine_live_progress_guard: RESULT=PASS sidecar_max_rss_mb=${SIDECAR_MAX} press=1 progress=1 complete=1 (NOT product Begin/Esc/clock PASS)"
	exit 0
fi
echo "eoa_ix1_spine_live_progress_guard: RESULT=FAIL (exit=$CODE sidecar_max_rss_mb=${SIDECAR_MAX} press=$PRESS_OK progress=$PROG_OK complete=$COMPLETE_OK rss_ok=$RSS_OK godot_fail=$GODOT_FAIL)"
exit 1
