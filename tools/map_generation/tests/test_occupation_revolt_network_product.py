#!/usr/bin/env python3
"""Gates: occupation revolt network product (major #58)."""
from __future__ import annotations
import sys, unittest
from pathlib import Path
ROOT = Path(__file__).resolve().parents[3]
sys.path.insert(0, str(ROOT / "tools" / "map_generation" / "lib"))
from occupation_revolt_network_product import *  # noqa
...
class TestPure(unittest.TestCase):
    def test_product(self):
        p = build_occupation_revolt_network_product()
        self.assertFalse(p.get("empty"))
        self.assertGreaterEqual(len(p.get("day_rows") or []), 3)
        self.assertGreaterEqual(int(p.get("cell_n", 0)), 3)
        self.assertGreaterEqual(int(p.get("province_n", 0)), 8)
        self.assertTrue(close_occupation_revolt_network_product_loop().get("ok"))
        for s in PRODUCT_STEPS:
            self.assertTrue(execute_revolt_network_step(s).get("ok"))
        self.assertTrue(occupation_revolt_network_integrity().get("ok"))
...
class TestLive(unittest.TestCase):
    def test_stack(self):
        ...
        self.assertIn("static func occupation_revolt_network_product(", fmt)
        self.assertIn("func occupation_revolt_network_product_live", mm)
        self.assertIn("func apply_revolt_network_live", gd)
        self.assertIn("Occupation revolt network (major #58)", panel)
        self.assertIn("build_occupation_revolt_network_product_chip_bbcode", pi)
        self.assertIn("phase11_depth_live", sl)
        self.assertIn("test_occupation_revolt_network_product.py", (ROOT / "tools/run_map_ci.sh").read_text())
    def test_docs(self):
        for path in (ROOT/"TODO.md", ROOT/"Project_State_Summary.md", ROOT/"Next_30_Days_Roadmap.md", ROOT/"GAME_STATUS_ASSESSMENT.md"):
            low = path.read_text().lower()
            for lab in ('revolt network', 'major #58', 'phase11_depth', 'world_class_gs'):
