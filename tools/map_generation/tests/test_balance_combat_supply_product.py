#!/usr/bin/env python3
"""Gates: balance combat/supply product (major #43)."""
from __future__ import annotations
import sys, unittest
from pathlib import Path
ROOT = Path(__file__).resolve().parents[3]
sys.path.insert(0, str(ROOT / "tools" / "map_generation" / "lib"))
from balance_combat_supply_product import *  # noqa
...
class TestPure(unittest.TestCase):
    def test_product(self):
        p = build_balance_combat_supply_product()
        self.assertFalse(p.get("empty"))
        self.assertGreaterEqual(len(p.get("day_rows") or []), 3)
        self.assertTrue(bool(p.get("within_band", False)) or float(p.get("variance", 1.0)) <= 0.25)
        for s in PRODUCT_STEPS:
            self.assertTrue(execute_balance_step(s).get("ok"))
        self.assertTrue(balance_combat_supply_integrity().get("ok"))
        self.assertTrue(close_balance_combat_supply_product_loop().get("ok"))
...
class TestLive(unittest.TestCase):
    def test_stack(self):
        ...
        self.assertIn("static func balance_combat_supply_product(", fmt)
        self.assertIn("func balance_combat_supply_product_live", mm)
        self.assertIn("func apply_balance_live", gd)
        self.assertIn("Balance combat/supply (major #43)", panel)
        self.assertIn("build_balance_combat_supply_product_chip_bbcode", pi)
        self.assertIn("phase6_depth_live", sl)
        self.assertIn("test_balance_combat_supply_product.py", (ROOT / "tools/run_map_ci.sh").read_text())
    def test_docs(self):
        for path in (ROOT/"TODO.md", ROOT/"Project_State_Summary.md", ROOT/"Next_30_Days_Roadmap.md", ROOT/"GAME_STATUS_ASSESSMENT.md"):
            low = path.read_text().lower()
            for lab in ("balance combat", "major #43", "phase6_depth", "supply"):
