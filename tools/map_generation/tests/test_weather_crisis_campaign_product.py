#!/usr/bin/env python3
"""Gates: weather crisis campaign product (major #50)."""
from __future__ import annotations
import sys, unittest
from pathlib import Path
ROOT = Path(__file__).resolve().parents[3]
sys.path.insert(0, str(ROOT / "tools" / "map_generation" / "lib"))
from weather_crisis_campaign_product import *  # noqa
...
class TestPure(unittest.TestCase):
    def test_product(self):
        p = build_weather_crisis_campaign_product()
        self.assertFalse(p.get("empty"))
        self.assertGreaterEqual(len(p.get("day_rows") or []), 3)
        self.assertGreaterEqual(int(p.get("fronts", 0)), 1)
        for s in PRODUCT_STEPS:
            self.assertTrue(execute_weather_crisis_step(s).get("ok"))
        self.assertTrue(weather_crisis_campaign_integrity().get("ok"))
        self.assertTrue(close_weather_crisis_campaign_product_loop().get("ok"))
...
class TestLive(unittest.TestCase):
    def test_stack(self):
        ...
        self.assertIn("static func weather_crisis_campaign_product(", fmt)
        self.assertIn("func weather_crisis_campaign_product_live", mm)
        self.assertIn("func apply_weather_crisis_live", gd)
        self.assertIn("Weather crisis campaign (major #50)", panel)
        self.assertIn("build_weather_crisis_campaign_product_chip_bbcode", pi)
        self.assertIn("phase9_cycle_live", sl)
        self.assertIn("test_weather_crisis_campaign_product.py", (ROOT / "tools/run_map_ci.sh").read_text())
    def test_docs(self):
        for path in (ROOT/"TODO.md", ROOT/"Project_State_Summary.md", ROOT/"Next_30_Days_Roadmap.md", ROOT/"GAME_STATUS_ASSESSMENT.md"):
            low = path.read_text().lower()
            for lab in ("weather crisis", "major #50", "phase9_cycle", "full gameplay cycle"):
