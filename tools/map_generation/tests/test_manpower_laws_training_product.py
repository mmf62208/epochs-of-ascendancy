#!/usr/bin/env python3
"""Gates: manpower laws/training product (major #30)."""
from __future__ import annotations
import sys, unittest
from pathlib import Path
ROOT = Path(__file__).resolve().parents[3]
sys.path.insert(0, str(ROOT / "tools" / "map_generation" / "lib"))
from manpower_laws_training_product import *  # noqa
...
class TestPure(unittest.TestCase):
    def test_product(self):
        p = build_manpower_laws_training_product()
        ...
        for s in PRODUCT_STEPS:
            self.assertTrue(execute_manpower_law_step(s).get("ok"))
        self.assertTrue(manpower_laws_training_integrity().get("ok"))
        self.assertTrue(close_manpower_laws_training_product_loop().get("ok"))
...
class TestLive(unittest.TestCase):
    def test_stack(self):
        ...
        self.assertIn("static func manpower_laws_training_product(", fmt)
        self.assertIn("func manpower_laws_training_product_live", mm)
        self.assertIn("func apply_manpower_law_live", gd)
        self.assertIn("func apply_manpower_training_tick_live", gd)
        self.assertIn("Manpower laws/training (major #30)", panel)
        self.assertIn("build_manpower_laws_training_product_chip_bbcode", pi)
        self.assertIn("phase2_conquest_live", sl)
        self.assertIn("test_manpower_laws_training_product.py", (ROOT / "tools/run_map_ci.sh").read_text())

    def test_docs(self):
        for path in (ROOT / "TODO.md", ROOT / "Project_State_Summary.md", ROOT / "Next_30_Days_Roadmap.md", ROOT / "GAME_STATUS_ASSESSMENT.md"):
            low = path.read_text().lower()
            for lab in ("manpower laws", "manpower training", "major #30", "phase2_conquest"):
