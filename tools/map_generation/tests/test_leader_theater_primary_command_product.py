...
from leader_theater_primary_command_product import LIVE_API_BY_STEP, build_leader_theater_primary_command_product, close_leader_theater_primary_command_package, leader_theater_primary_command_integrity, primary_command_dead_audit
...
class T(unittest.TestCase):
    ...
    def test_close_integrity(self):
        self.assertTrue(close_leader_theater_primary_command_package().get("ok"))
    ...
    def test_plain(self):
        self.assertTrue(str(build_leader_theater_primary_command_product(province_id=2).get("plain") or "").strip())
