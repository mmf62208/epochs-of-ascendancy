#!/usr/bin/env python3
"""Gates: war economy conversion product (major #46)."""
from __future__ import annotations
import sys, unittest
from pathlib import Path
ROOT = Path(__file__).resolve().parents[3]
sys.path.insert(0, str(ROOT / "tools" / "map_generation" / "lib"))
from war_economy_conversion_product import *  # noqa
...
class TestPure(unittest.TestCase):
    def test_product(self):
        p = build_war_economy_conversion_product()
        self.assertFalse(p.get("empty"))
        self.assertGreaterEqual(len(p.get("day_rows") or []), 3)
        self.assertGreaterEqual(int(p.get("factories", 0)), 1)
        for s in PRODUCT_STEPS:
            self.assertTrue(execute_economy_conversion_step(s).get("ok"))
        self.assertTrue(war_economy_conversion_integrity().get("ok"))
        self.assertTrue(close_war_economy_conversion_product_loop().get("ok"))
...
class TestLive(unittest.TestCase):
    def test_stack(self):
        ...
        self.assertIn("static func war_economy_conversion_product(", fmt)
        self.assertIn("func war_economy_conversion_product_live", mm)
        self.assertIn("func apply_economy_conversion_live", gd)
        self.assertIn("War economy conversion (major #46)", panel)
        self.assertIn("build_war_economy_conversion_product_chip_bbcode", pi)
        self.assertIn("phase7_depth_live", sl)
        self.assertIn("test_war_economy_conversion_product.py", (ROOT / "tools/run_map_ci.sh").read_text())
    def test_docs(self):
        for path in (ROOT/"TODO.md", ROOT/"Project_State_Summary.md", ROOT/"Next_30_Days_Roadmap.md", ROOT/"GAME_STATUS_ASSESSMENT.md"):
            low = path.read_text().lower()
            for lab in ("war economy conversion", "major #46", "phase7_depth", "economy"):
