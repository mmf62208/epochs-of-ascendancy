#!/usr/bin/env python3
"""Gates: focus tree content product (major #42)."""
from __future__ import annotations
import sys, unittest
from pathlib import Path
ROOT = Path(__file__).resolve().parents[3]
sys.path.insert(0, str(ROOT / "tools" / "map_generation" / "lib"))
from focus_tree_content_product import *  # noqa
...
class TestPure(unittest.TestCase):
    def test_product(self):
        p = build_focus_tree_content_product()
        self.assertFalse(p.get("empty"))
        self.assertGreaterEqual(len(p.get("day_rows") or []), 3)
        self.assertGreaterEqual(int(p.get("focus_n", 0)), 1)
        for s in PRODUCT_STEPS:
            self.assertTrue(execute_focus_tree_content_step(s).get("ok"))
        self.assertTrue(focus_tree_content_integrity().get("ok"))
        self.assertTrue(close_focus_tree_content_product_loop().get("ok"))
...
class TestLive(unittest.TestCase):
    def test_stack(self):
        ...
        self.assertIn("static func focus_tree_content_product(", fmt)
        self.assertIn("func focus_tree_content_product_live", mm)
        self.assertIn("func apply_focus_content_live", gd)
        self.assertIn("Focus tree content (major #42)", panel)
        self.assertIn("build_focus_tree_content_product_chip_bbcode", pi)
        self.assertIn("phase6_depth_live", sl)
        self.assertIn("test_focus_tree_content_product.py", (ROOT / "tools/run_map_ci.sh").read_text())
    def test_docs(self):
        for path in (ROOT/"TODO.md", ROOT/"Project_State_Summary.md", ROOT/"Next_30_Days_Roadmap.md", ROOT/"GAME_STATUS_ASSESSMENT.md"):
            low = path.read_text().lower()
            for lab in ("focus tree content", "major #42", "phase6_depth", "focus"):
