#!/usr/bin/env python3
"""Gates: grand strategy cycle product (major #55)."""
from __future__ import annotations
import sys, unittest
from pathlib import Path
ROOT = Path(__file__).resolve().parents[3]
sys.path.insert(0, str(ROOT / "tools" / "map_generation" / "lib"))
from grand_strategy_cycle_product import *  # noqa
...
class TestPure(unittest.TestCase):
    def test_product(self):
        p = build_grand_strategy_cycle_product()
        self.assertFalse(p.get("empty"))
        self.assertGreaterEqual(len(p.get("day_rows") or []), 3)
        self.assertGreaterEqual(int(p.get("open_n", 0)), 5)
        self.assertTrue(bool(p.get("complete")))
        self.assertTrue(close_grand_strategy_cycle_product_loop().get("ok"))
        for s in PRODUCT_STEPS:
            self.assertTrue(execute_gs_cycle_step(s).get("ok"))
        self.assertTrue(grand_strategy_cycle_integrity().get("ok"))
...
class TestLive(unittest.TestCase):
    def test_stack(self):
        ...
        self.assertIn("static func grand_strategy_cycle_product(", fmt)
        self.assertIn("func grand_strategy_cycle_product_live", mm)
        self.assertIn("func apply_gs_cycle_live", gd)
        self.assertIn("Grand strategy cycle (major #55)", panel)
        self.assertIn("build_grand_strategy_cycle_product_chip_bbcode", pi)
        self.assertIn("phase10_gs_live", sl)
        self.assertIn("test_grand_strategy_cycle_product.py", (ROOT / "tools/run_map_ci.sh").read_text())
    def test_docs(self):
        for path in (ROOT/"TODO.md", ROOT/"Project_State_Summary.md", ROOT/"Next_30_Days_Roadmap.md", ROOT/"GAME_STATUS_ASSESSMENT.md"):
            low = path.read_text().lower()
            for lab in ('grand strategy', 'major #55', 'phase10_gs', 'world_class_gs'):
