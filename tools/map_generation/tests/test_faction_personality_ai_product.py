#!/usr/bin/env python3
"""Gates: faction personality AI product (major #57)."""
from __future__ import annotations
import sys, unittest
from pathlib import Path
ROOT = Path(__file__).resolve().parents[3]
sys.path.insert(0, str(ROOT / "tools" / "map_generation" / "lib"))
from faction_personality_ai_product import *  # noqa
...
class TestPure(unittest.TestCase):
    def test_product(self):
        p = build_faction_personality_ai_product()
        self.assertFalse(p.get("empty"))
        self.assertGreaterEqual(len(p.get("day_rows") or []), 3)
        self.assertGreaterEqual(int(p.get("personality_n", 0)), 5)
        self.assertTrue(close_faction_personality_ai_product_loop().get("ok"))
        for s in PRODUCT_STEPS:
            self.assertTrue(execute_personality_step(s).get("ok"))
        self.assertTrue(faction_personality_ai_integrity().get("ok"))
...
class TestLive(unittest.TestCase):
    def test_stack(self):
        ...
        self.assertIn("static func faction_personality_ai_product(", fmt)
        self.assertIn("func faction_personality_ai_product_live", mm)
        self.assertIn("func apply_personality_live", gd)
        self.assertIn("Faction personality AI (major #57)", panel)
        self.assertIn("build_faction_personality_ai_product_chip_bbcode", pi)
        self.assertIn("phase11_depth_live", sl)
        self.assertIn("test_faction_personality_ai_product.py", (ROOT / "tools/run_map_ci.sh").read_text())
    def test_docs(self):
        for path in (ROOT/"TODO.md", ROOT/"Project_State_Summary.md", ROOT/"Next_30_Days_Roadmap.md", ROOT/"GAME_STATUS_ASSESSMENT.md"):
            low = path.read_text().lower()
            for lab in ('personality', 'major #57', 'phase11_depth', 'world_class_gs'):
