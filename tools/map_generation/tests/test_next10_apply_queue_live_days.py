#!/usr/bin/env python3
"""Gates: next-370 apply-queue live depth (10)."""
from __future__ import annotations
import sys, unittest
from pathlib import Path
ROOT = Path(__file__).resolve().parents[3]
sys.path.insert(0, str(ROOT / "tools" / "map_generation" / "lib"))
from next370_apply_queue_live import *  # noqa
from gis_coastline_ingest import load_geometry_payload  # noqa

LIVE = set(APPLY_QUEUE_LIVE_DAY_IDS[:6])
...
class TestPackages(unittest.TestCase):
    def test_ten(self):
        self.assertEqual(len(APPLY_QUEUE_LIVE_DAY_IDS), 10)
    ...
    def test_close(self):
        self.assertTrue(close_next370_apply_queue_live_loop().get("ok"))
        self.assertTrue(apply_queue_live_depth_integrity().get("ok"))
...
class TestLive(unittest.TestCase):
    def test_wiring(self):
        ...
        for aid in APPLY_QUEUE_LIVE_DAY_IDS:
        ...
        self.assertIn("_next370_live_day", mm)
        self.assertIn("func _rebuild_apply_queue_live_section", panel)
        self.assertIn("test_next10_apply_queue_live_days.py", (ROOT / "tools/run_map_ci.sh").read_text())
        labels = [
            "apply queue audit day", "apply queue production live day", "apply queue combat live day",
            "apply queue supply live day", "apply queue focus live day", "apply queue agent live day",
            "apply queue station live day", "apply queue six leaf joint day", "apply queue honesty joint day",
            "apply queue live managers close day", "next-370",
        ]
