"""Campaign AI multi-month primary package — Master Plan residual.
product → board → weekly → execute → close.
...
def _compose(pid):
    try:
        return build_campaign_ai_multi_month_product(province_id=pid)
    except TypeError:
        try:
            return build_campaign_ai_multi_month_product()
        except Exception:
            return {"score": 0.75, "empty": False}
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "CAM primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_campaign_ai_multi_month_primary_command_product(*, province_id: int = 1, live_ids=None):
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "CAM · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "CAM primary · majors %d/5 · dead %d · score %.2f · %s" % (
    ...
    return {
        ...
        "integration": ["campaign_ai_multi_month_primary_command_product", "campaign_ai_multi_month_product", "CAM"],
    }


def apply_campaign_ai_multi_month_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    ...
    product = build_campaign_ai_multi_month_primary_command_product(province_id=province_id)
...
def close_campaign_ai_multi_month_primary_command_package(province_id=1):
    rt = {"applied": []}
    steps = [apply_campaign_ai_multi_month_primary_command_step(s, province_id, runtime=rt) for s in PRIMARY_COMMAND_STEPS]
    product = build_campaign_ai_multi_month_primary_command_product(province_id=province_id)
    ...
    return {"ok": ok, "live": True, "product": product, "steps": steps, "applied_n": len(rt["applied"]),
            "dead_n": int(product.get("dead_n") or 0), "summary": "CAM close %s" % ("PASS" if ok else "FAIL"),
            "plain": "CAM close", "empty": False}


def campaign_ai_multi_month_primary_command_integrity():
    p = build_campaign_ai_multi_month_primary_command_product()
    c = close_campaign_ai_multi_month_primary_command_package()
    ...
    return {"ok": ok, "no_focus": no_focus, "summary": "CAM integrity %s" % ("PASS" if ok else "FAIL"), "empty": False}
