"""Faction personality primary package — Master Plan A4 residual.\nboard → event → drive → product → close.
...
try:
    from faction_personality_ai_product import build_faction_personality_ai_product  # type: ignore
except Exception:  # pragma: no cover
    def build_faction_personality_ai_product(*_a, **_k):  # type: ignore
        return {"score": 0.62, "empty": False}

def _compose(pid):
    try:
        return build_faction_personality_ai_product(province_id=pid)
    except TypeError:
        return build_faction_personality_ai_product()
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "A4 primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_faction_personality_primary_command_product(*, province_id: int = 1, live_ids=None):
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "A4 · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "A4 primary · majors %d/5 · dead %d · score %.2f · %s" % (
    ...
    return {
        ...
        "integration": ['faction_personality_primary_command_product', 'faction_personality_ai_product', 'A4'],
    }


def apply_faction_personality_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    ...
    product = build_faction_personality_primary_command_product(province_id=province_id)
...
def close_faction_personality_primary_command_package(province_id=1):
    rt = {"applied": []}
    steps = [apply_faction_personality_primary_command_step(s, province_id, runtime=rt) for s in PRIMARY_COMMAND_STEPS]
    product = build_faction_personality_primary_command_product(province_id=province_id)
    ...
    return {"ok": ok, "live": True, "product": product, "steps": steps, "applied_n": len(rt["applied"]),
            "dead_n": int(product.get("dead_n") or 0), "summary": "A4 close %s" % ("PASS" if ok else "FAIL"),
            "plain": "A4 close", "empty": False}


def faction_personality_primary_command_integrity():
    p = build_faction_personality_primary_command_product()
    c = close_faction_personality_primary_command_package()
    ...
    return {"ok": ok, "no_focus": no_focus, "summary": "A4 integrity %s" % ("PASS" if ok else "FAIL"), "empty": False}
