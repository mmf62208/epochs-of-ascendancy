"""Fleet multi-day autonomy primary package — Master Plan C2 residual.\nproduct → posture → escort → follow → sequence.
...
def _compose(pid):
    try:
        return build_fleet_multi_day_autonomy_product(province_id=pid)
    except TypeError:
        return build_fleet_multi_day_autonomy_product()
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "FMD primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_fleet_multi_day_primary_command_product(*, province_id: int = 1, live_ids=None):
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "FMD · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "FMD primary · majors %d/5 · dead %d · score %.2f · %s" % (
    ...
    return {
        ...
        "integration": ['fleet_multi_day_primary_command_product', 'fleet_multi_day_autonomy_product', 'C2'],
    }


def apply_fleet_multi_day_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    ...
    product = build_fleet_multi_day_primary_command_product(province_id=province_id)
...
def close_fleet_multi_day_primary_command_package(province_id=1):
    rt = {"applied": []}
    steps = [apply_fleet_multi_day_primary_command_step(s, province_id, runtime=rt) for s in PRIMARY_COMMAND_STEPS]
    product = build_fleet_multi_day_primary_command_product(province_id=province_id)
    ...
    return {"ok": ok, "live": True, "product": product, "steps": steps, "applied_n": len(rt["applied"]),
            "dead_n": int(product.get("dead_n") or 0), "summary": "FMD close %s" % ("PASS" if ok else "FAIL"),
            "plain": "FMD close", "empty": False}


def fleet_multi_day_primary_command_integrity():
    p = build_fleet_multi_day_primary_command_product()
    c = close_fleet_multi_day_primary_command_package()
    ...
    return {"ok": ok, "no_focus": no_focus, "summary": "FMD integrity %s" % ("PASS" if ok else "FAIL"), "empty": False}
