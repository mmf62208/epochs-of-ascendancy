"""Q1 checklist depth primary.
LIVE_API = real GameData methods (not bare apply_focus).
"""
from __future__ import annotations
SURFACE_KEYS = ('q1c_primary_balance', 'q1c_primary_variance', 'q1c_primary_prove60', 'q1c_primary_prove100', 'q1c_primary_close')
PRIMARY_COMMAND_STEPS = ('q1c_balance', 'q1c_variance', 'q1c_prove60', 'q1c_prove100', 'q1c_close')
_STEP_MAJOR = {s: SURFACE_KEYS[i] for i, s in enumerate(PRIMARY_COMMAND_STEPS)}
LIVE_API_BY_STEP = {'q1c_balance': 'apply_balance_combat_supply_product', 'q1c_variance': 'apply_balance_variance_close', 'q1c_prove60': 'apply_medium_honesty_prove_60d', 'q1c_prove100': 'apply_medium_honesty_prove_100d', 'q1c_close': 'apply_medium_tank_production_honesty_close_day'}
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label="Q1 checklist depth primary audit · dead %d · %s"%(len(dead),"PASS" if ok else "FAIL")
    return {"action_ids":ids,"dead":dead,"dead_n":len(dead),"ok":ok,"summary":label,"plain":label,"empty":False}

def build_q1_checklist_primary_command_product(*, province_id=1, live_ids=None):
    ...
    for i,step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        lab="q1_checklist · %s · live %s · score %.2f"%(step,api,sc)
...
def apply_q1_checklist_primary_command_step(step, province_id=1, *, runtime=None):
    ...
    product=build_q1_checklist_primary_command_product(province_id=province_id)
