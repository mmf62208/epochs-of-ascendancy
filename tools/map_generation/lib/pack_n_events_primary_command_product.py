"""Pack N events/focus primary — residual catalog product.
LIVE_API = real GameData methods (not bare apply_focus).
"""
from __future__ import annotations
from typing import Any, Dict

SURFACE_KEYS = ('pnevt_primary_catalog', 'pnevt_primary_path', 'pnevt_primary_war_pick', 'pnevt_primary_commit', 'pnevt_primary_close')
PRIMARY_COMMAND_STEPS = ('pnevt_catalog', 'pnevt_path', 'pnevt_war_pick', 'pnevt_commit', 'pnevt_close')
_STEP_MAJOR = {s: SURFACE_KEYS[i] for i, s in enumerate(PRIMARY_COMMAND_STEPS)}
LIVE_API_BY_STEP = {'pnevt_catalog': 'apply_focus_tree_catalog', 'pnevt_path': 'apply_focus_tree_path', 'pnevt_war_pick': 'apply_focus_war_pick', 'pnevt_commit': 'apply_focus_war_commit', 'pnevt_close': 'apply_focus_tree_content_close_day'}
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "Pack N events/focus primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_pack_n_events_primary_command_product(*, province_id: int = 1, live_ids=None):
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "PackNEvents · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "Pack N events/focus primary · majors %d/5 · dead %d · score %.2f · %s" % (majors_ok_n, dead_n, score, "PASS" if all_majors_ok else "PARTIAL")
    return {
        ...
        "apply_queue": apply_queue, "integration": ["pack_n_events_primary_command_product", "PackNEvents"],
    }


def apply_pack_n_events_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    ...
    product = build_pack_n_events_primary_command_product(province_id=province_id)
