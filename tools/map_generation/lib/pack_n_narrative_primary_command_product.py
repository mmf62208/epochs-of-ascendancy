"""Pack N narrative primary.
LIVE_API = real GameData methods (not bare apply_focus).
"""
from __future__ import annotations
SURFACE_KEYS = ('pnn_primary_event', 'pnn_primary_catalog', 'pnn_primary_path', 'pnn_primary_commit', 'pnn_primary_close')
PRIMARY_COMMAND_STEPS = ('pnn_event', 'pnn_catalog', 'pnn_path', 'pnn_commit', 'pnn_close')
_STEP_MAJOR = {s: SURFACE_KEYS[i] for i, s in enumerate(PRIMARY_COMMAND_STEPS)}
LIVE_API_BY_STEP = {'pnn_event': 'apply_personality_event', 'pnn_catalog': 'apply_focus_tree_catalog', 'pnn_path': 'apply_focus_tree_path', 'pnn_commit': 'apply_focus_tree_commit', 'pnn_close': 'apply_focus_war_path_close_day'}
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label="Pack N narrative primary audit · dead %d · %s"%(len(dead),"PASS" if ok else "FAIL")
    return {"action_ids":ids,"dead":dead,"dead_n":len(dead),"ok":ok,"summary":label,"plain":label,"empty":False}

def build_pack_n_narrative_primary_command_product(*, province_id=1, live_ids=None):
    ...
    for i,step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        lab="pack_n_narrative · %s · live %s · score %.2f"%(step,api,sc)
...
def apply_pack_n_narrative_primary_command_step(step, province_id=1, *, runtime=None):
    ...
    product=build_pack_n_narrative_primary_command_product(province_id=province_id)
