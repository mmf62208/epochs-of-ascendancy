"""Front continuity primary package — Master Plan C residual.\ncampaign → combat → assault → sustain → close.
...
def _compose(pid):
    try:
        return build_front_continuity_campaign_product(province_id=pid)
    except TypeError:
        return build_front_continuity_campaign_product()
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "FC primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_front_continuity_primary_command_product(*, province_id: int = 1, live_ids=None):
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "FC · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "FC primary · majors %d/5 · dead %d · score %.2f · %s" % (
    ...
    return {
        ...
        "integration": ['front_continuity_primary_command_product', 'front_continuity_campaign_product', 'C'],
    }


def apply_front_continuity_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    ...
    product = build_front_continuity_primary_command_product(province_id=province_id)
...
def close_front_continuity_primary_command_package(province_id=1):
    rt = {"applied": []}
    steps = [apply_front_continuity_primary_command_step(s, province_id, runtime=rt) for s in PRIMARY_COMMAND_STEPS]
    product = build_front_continuity_primary_command_product(province_id=province_id)
    ...
    return {"ok": ok, "live": True, "product": product, "steps": steps, "applied_n": len(rt["applied"]),
            "dead_n": int(product.get("dead_n") or 0), "summary": "FC close %s" % ("PASS" if ok else "FAIL"),
            "plain": "FC close", "empty": False}


def front_continuity_primary_command_integrity():
    p = build_front_continuity_primary_command_product()
    c = close_front_continuity_primary_command_package()
    ...
    return {"ok": ok, "no_focus": no_focus, "summary": "FC integrity %s" % ("PASS" if ok else "FAIL"), "empty": False}
