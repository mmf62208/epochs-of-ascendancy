"""HOI fullscreen surface primary.
LIVE_API = real GameData methods (not bare apply_focus).
"""
from __future__ import annotations
SURFACE_KEYS = ('hfs_primary_compact', 'hfs_primary_inspector', 'hfs_primary_apply', 'hfs_primary_polish', 'hfs_primary_close')
PRIMARY_COMMAND_STEPS = ('hfs_compact', 'hfs_inspector', 'hfs_apply', 'hfs_polish', 'hfs_close')
_STEP_MAJOR = {s: SURFACE_KEYS[i] for i, s in enumerate(PRIMARY_COMMAND_STEPS)}
LIVE_API_BY_STEP = {'hfs_compact': 'apply_product_ux_compact_board', 'hfs_inspector': 'apply_inspector_decision_product', 'hfs_apply': 'apply_inspector_product_apply', 'hfs_polish': 'apply_product_ux_command_polish_product', 'hfs_close': 'apply_product_ux_polish_close_day'}
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label="HOI fullscreen surface primary audit · dead %d · %s"%(len(dead),"PASS" if ok else "FAIL")
    return {"action_ids":ids,"dead":dead,"dead_n":len(dead),"ok":ok,"summary":label,"plain":label,"empty":False}

def build_hoi_fullscreen_primary_command_product(*, province_id=1, live_ids=None):
    ...
    for i,step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        lab="hoi_fullscreen · %s · live %s · score %.2f"%(step,api,sc)
...
def apply_hoi_fullscreen_primary_command_step(step, province_id=1, *, runtime=None):
    ...
    product=build_hoi_fullscreen_primary_command_product(province_id=province_id)
