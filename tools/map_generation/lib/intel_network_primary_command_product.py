"""Intel network primary package — Master Plan I2.
coverage → counterintel → counterplay → product → close.
...
SURFACE_KEYS = (
    "intel_primary_coverage",
    "intel_primary_counterintel",
    "intel_primary_counterplay",
    "intel_primary_product",
    "intel_primary_close",
)
...
PRIMARY_ACTION_IDS = tuple(LIVE_API_BY_STEP.values()) + (
    "apply_intel_cell_network_product",
    "apply_intel_cell_coverage",
    "apply_intel_cell_ops",
    "apply_intel_cell_network_close_day",
)
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "Intel network primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_intel_network_primary_command_product(*, province_id: int = 1, live_ids=None):
    ...
    try:
        ...
        cells = build_intel_cell_network_product()
    base = _floor(0.55 * float(net.get("score") or 0.6) + 0.45 * float(cells.get("score") or 0.55))
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "I2 · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "Intel network primary · majors %d/5 · dead %d · score %.2f · %s" % (
    ...
    return {
        ...
        "apply_queue": apply_queue, "network": net, "cells": cells,
        "integration": ["intel_network_primary_command_product", "intelligence_network_product", "I2", "intel"],
    }


def apply_intel_network_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    s = str(step or "").strip().lower()
    aliases = {"coverage": "intel_coverage", "counterintel": "intel_counterintel", "counterplay": "intel_counterplay",
               "product": "intel_product", "close": "intel_close"}
    s = aliases.get(s, s)
    if s not in PRIMARY_COMMAND_STEPS:
        s = PRIMARY_COMMAND_STEPS[0]
    product = build_intel_network_primary_command_product(province_id=province_id)
...
def close_intel_network_primary_command_package(province_id=1):
    rt = {"applied": []}
    steps = [apply_intel_network_primary_command_step(s, province_id, runtime=rt) for s in PRIMARY_COMMAND_STEPS]
    product = build_intel_network_primary_command_product(province_id=province_id)
    ...
    return {"ok": ok, "live": True, "product": product, "steps": steps, "applied_n": len(rt["applied"]),
            "dead_n": int(product.get("dead_n") or 0), "summary": "Intel network close %s" % ("PASS" if ok else "FAIL"),
            "plain": "Intel network close", "empty": False}


def intel_network_primary_command_integrity():
    p = build_intel_network_primary_command_product()
    c = close_intel_network_primary_command_package()
    ...
    return {"ok": ok, "no_focus": no_focus, "summary": "Intel network integrity %s" % ("PASS" if ok else "FAIL"), "empty": False}
