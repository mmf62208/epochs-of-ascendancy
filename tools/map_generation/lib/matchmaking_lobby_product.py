"""Matchmaking lobby primary product — board/lobby surface dual."""
from __future__ import annotations
from pathlib import Path
from typing import Any, Dict

ROOT = Path(__file__).resolve().parents[3]
SURFACE_KEYS = ('mml_primary_catalog', 'mml_primary_open', 'mml_primary_path', 'mml_primary_session', 'mml_primary_close')
PRIMARY_COMMAND_STEPS = ('mml_catalog', 'mml_open', 'mml_path', 'mml_session', 'mml_close')
_STEP_MAJOR = {s: SURFACE_KEYS[i] for i, s in enumerate(PRIMARY_COMMAND_STEPS)}
LIVE_API_BY_STEP = {'mml_catalog': 'apply_matchmaking_lobby_catalog_live', 'mml_open': 'apply_matchmaking_lobby_open_live', 'mml_path': 'apply_matchmaking_lobby_path_live', 'mml_session': 'apply_matchmaking_lobby_session_live', 'mml_close': 'apply_matchmaking_lobby_close_live'}
PRIMARY_ACTION_IDS = tuple(LIVE_API_BY_STEP.values())
LIVE_PRIMARY_ACTION_IDS = frozenset(PRIMARY_ACTION_IDS)
HOOKS = [('scripts/ui/MatchmakingLobbyView.gd', 'func apply_lobby_match_path'), ('scripts/ui/MatchmakingLobbyView.gd', 'func show_lobby'), ('scripts/ui/TopInfoBar.gd', 'func _on_matchmaking_pressed'), ('scripts/net/NetSessionManager.gd', 'func try_matchmake')]
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": len(dead)==0 and len(ids)>=5,
            "summary": "Matchmaking lobby audit", "plain": "Matchmaking lobby audit", "empty": False}

def build_matchmaking_lobby_primary_command_product(*, province_id: int = 1, live_ids=None) -> Dict[str, Any]:
    ...
    return {"score": 0.94 if all_ok else 0.4, "plain": label, "summary": label, "empty": False,
        ...
        "integration": ["matchmaking_lobby_product"]}
