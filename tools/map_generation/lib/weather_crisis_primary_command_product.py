"""Weather crisis campaign primary package — Master Plan residual.
product → forecast → gate_multi → sustain → close.
...
def _compose(pid):
    try:
        return build_weather_crisis_campaign_product(province_id=pid)
    except TypeError:
        try:
            return build_weather_crisis_campaign_product()
        except Exception:
            return {"score": 0.65, "empty": False}
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "WCR primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_weather_crisis_primary_command_product(*, province_id: int = 1, live_ids=None):
    pid = max(1, int(province_id))
    try:
        composed = _compose(pid)
    except Exception:
        composed = {"score": 0.65, "empty": False}
    base = _floor(float(composed.get("score") or 0.65))
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "WCR · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "WCR primary · majors %d/5 · dead %d · score %.2f · %s" % (
    ...
    return {
        ...
        "integration": ["weather_crisis_primary_command_product", "weather_crisis_campaign_product", "WCR"],
    }


def apply_weather_crisis_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    s = str(step or "").strip().lower()
    aliases = {}
    for full in PRIMARY_COMMAND_STEPS:
        aliases[full] = full
        aliases[full.split("_", 1)[-1]] = full
    aliases["gate_multi"] = "wcr_gate"
    s = aliases.get(s, s)
    if s not in PRIMARY_COMMAND_STEPS:
        s = PRIMARY_COMMAND_STEPS[0]
    product = build_weather_crisis_primary_command_product(province_id=province_id)
...
def close_weather_crisis_primary_command_package(province_id=1):
    rt = {"applied": []}
    steps = [apply_weather_crisis_primary_command_step(s, province_id, runtime=rt) for s in PRIMARY_COMMAND_STEPS]
    product = build_weather_crisis_primary_command_product(province_id=province_id)
    ...
    return {"ok": ok, "live": True, "product": product, "steps": steps, "applied_n": len(rt["applied"]),
            "dead_n": int(product.get("dead_n") or 0), "summary": "WCR close %s" % ("PASS" if ok else "FAIL"),
            "plain": "WCR close", "empty": False}


def weather_crisis_primary_command_integrity():
    p = build_weather_crisis_primary_command_product()
    c = close_weather_crisis_primary_command_package()
    ...
    return {"ok": ok, "no_focus": no_focus, "summary": "WCR integrity %s" % ("PASS" if ok else "FAIL"), "empty": False}
