"""Air multi-phase theater primary package — Master Plan C3 residual.\nproduct → sortie → weather → airland → close.
...
def _compose(pid):
    try:
        return build_air_multi_phase_theater_product(province_id=pid)
    except TypeError:
        return build_air_multi_phase_theater_product()
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "AMP primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_air_multi_phase_primary_command_product(*, province_id: int = 1, live_ids=None):
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "AMP · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "AMP primary · majors %d/5 · dead %d · score %.2f · %s" % (
    ...
    return {
        ...
        "integration": ['air_multi_phase_primary_command_product', 'air_multi_phase_theater_product', 'C3'],
    }


def apply_air_multi_phase_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    ...
    product = build_air_multi_phase_primary_command_product(province_id=province_id)
...
def close_air_multi_phase_primary_command_package(province_id=1):
    rt = {"applied": []}
    steps = [apply_air_multi_phase_primary_command_step(s, province_id, runtime=rt) for s in PRIMARY_COMMAND_STEPS]
    product = build_air_multi_phase_primary_command_product(province_id=province_id)
    ...
    return {"ok": ok, "live": True, "product": product, "steps": steps, "applied_n": len(rt["applied"]),
            "dead_n": int(product.get("dead_n") or 0), "summary": "AMP close %s" % ("PASS" if ok else "FAIL"),
            "plain": "AMP close", "empty": False}


def air_multi_phase_primary_command_integrity():
    p = build_air_multi_phase_primary_command_product()
    c = close_air_multi_phase_primary_command_package()
    ...
    return {"ok": ok, "no_focus": no_focus, "summary": "AMP integrity %s" % ("PASS" if ok else "FAIL"), "empty": False}
