"""Q1 validator honesty primary — residual catalog product.
LIVE_API = real GameData methods (not bare apply_focus).
"""
from __future__ import annotations
from typing import Any, Dict

SURFACE_KEYS = ('q1_primary_balance', 'q1_primary_estimate', 'q1_primary_sample', 'q1_primary_honesty', 'q1_primary_close')
PRIMARY_COMMAND_STEPS = ('q1_balance', 'q1_estimate', 'q1_sample', 'q1_honesty', 'q1_close')
_STEP_MAJOR = {s: SURFACE_KEYS[i] for i, s in enumerate(PRIMARY_COMMAND_STEPS)}
LIVE_API_BY_STEP = {'q1_balance': 'apply_balance_combat_supply_product', 'q1_estimate': 'apply_balance_estimate_board', 'q1_sample': 'apply_balance_live_sample', 'q1_honesty': 'apply_medium_honesty_prove_60d', 'q1_close': 'apply_balance_combat_supply_close_day'}
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "Q1 validator honesty primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_q1_validator_primary_command_product(*, province_id: int = 1, live_ids=None):
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "Q1Validator · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "Q1 validator honesty primary · majors %d/5 · dead %d · score %.2f · %s" % (majors_ok_n, dead_n, score, "PASS" if all_majors_ok else "PARTIAL")
    return {
        ...
        "apply_queue": apply_queue, "integration": ["q1_validator_primary_command_product", "Q1Validator"],
    }


def apply_q1_validator_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    ...
    product = build_q1_validator_primary_command_product(province_id=province_id)
