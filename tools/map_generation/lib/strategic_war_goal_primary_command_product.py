"""Strategic war-goal primary package — Master Plan residual (major #53).
product → board → justify → execute → close.
LIVE_API = real GameData methods (not bare apply_focus).
Distinct from war_goal_alliance_primary.
"""
...
def _compose(pid):
    try:
        return build_strategic_war_goal_product(province_id=pid)
    except TypeError:
        try:
            return build_strategic_war_goal_product()
        except Exception:
            return {"score": 0.7, "empty": False}
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "SWG primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_strategic_war_goal_primary_command_product(*, province_id: int = 1, live_ids=None):
    pid = max(1, int(province_id))
    try:
        composed = _compose(pid)
    except Exception:
        composed = {"score": 0.7, "empty": False}
    base = _floor(float(composed.get("score") or 0.7))
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "SWG · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "SWG primary · majors %d/5 · dead %d · score %.2f · %s" % (
    ...
    return {
        ...
        "integration": ["strategic_war_goal_primary_command_product", "strategic_war_goal_product", "major_53", "SWG"],
    }


def apply_strategic_war_goal_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    ...
    product = build_strategic_war_goal_primary_command_product(province_id=province_id)
...
def close_strategic_war_goal_primary_command_package(province_id=1):
    rt = {"applied": []}
    steps = [apply_strategic_war_goal_primary_command_step(s, province_id, runtime=rt) for s in PRIMARY_COMMAND_STEPS]
    product = build_strategic_war_goal_primary_command_product(province_id=province_id)
    ...
    return {"ok": ok, "live": True, "product": product, "steps": steps, "applied_n": len(rt["applied"]),
            "dead_n": int(product.get("dead_n") or 0), "summary": "SWG close %s" % ("PASS" if ok else "FAIL"),
            "plain": "SWG close", "empty": False}


def strategic_war_goal_primary_command_integrity():
    p = build_strategic_war_goal_primary_command_product()
    c = close_strategic_war_goal_primary_command_package()
    ...
    return {"ok": ok, "no_focus": no_focus, "summary": "SWG integrity %s" % ("PASS" if ok else "FAIL"), "empty": False}
