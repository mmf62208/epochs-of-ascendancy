"""Production honesty primary package — Master Plan P3 residual.\nproduct → prove60 → prove100 → oob → close.
...
def _compose(pid):
    try:
        return build_medium_tank_production_honesty_product(province_id=pid)
    except TypeError:
        return build_medium_tank_production_honesty_product()
...
LIVE_API_BY_STEP = {
    "ph_product": "apply_medium_tank_production_honesty_product",
    "ph_prove60": "apply_medium_honesty_prove_60d",
    "ph_prove100": "apply_medium_honesty_prove_100d",
    "ph_oob": "apply_medium_tank_oob_product",
    "ph_close": "apply_medium_tank_production_honesty_close_day",
}
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "P3 primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_production_honesty_primary_command_product(*, province_id: int = 1, live_ids=None):
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "P3 · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "P3 primary · majors %d/5 · dead %d · score %.2f · %s" % (
    ...
    return {
        ...
        "integration": ['production_honesty_primary_command_product', 'medium_tank_production_honesty_product', 'P3'],
    }


def apply_production_honesty_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    ...
    product = build_production_honesty_primary_command_product(province_id=province_id)
...
def close_production_honesty_primary_command_package(province_id=1):
    rt = {"applied": []}
    steps = [apply_production_honesty_primary_command_step(s, province_id, runtime=rt) for s in PRIMARY_COMMAND_STEPS]
    product = build_production_honesty_primary_command_product(province_id=province_id)
    ...
    return {"ok": ok, "live": True, "product": product, "steps": steps, "applied_n": len(rt["applied"]),
            "dead_n": int(product.get("dead_n") or 0), "summary": "P3 close %s" % ("PASS" if ok else "FAIL"),
            "plain": "P3 close", "empty": False}


def production_honesty_primary_command_integrity():
    p = build_production_honesty_primary_command_product()
    c = close_production_honesty_primary_command_package()
    ...
    return {"ok": ok, "no_focus": no_focus, "summary": "P3 integrity %s" % ("PASS" if ok else "FAIL"), "empty": False}
