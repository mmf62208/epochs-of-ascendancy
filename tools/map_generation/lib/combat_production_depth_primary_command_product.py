"""Combat/production depth primary.
LIVE_API = real GameData methods (not bare apply_focus).
"""
from __future__ import annotations
SURFACE_KEYS = ('cpd_primary_estimate', 'cpd_primary_phase', 'cpd_primary_joint', 'cpd_primary_fleet', 'cpd_primary_close')
PRIMARY_COMMAND_STEPS = ('cpd_estimate', 'cpd_phase', 'cpd_joint', 'cpd_fleet', 'cpd_close')
_STEP_MAJOR = {s: SURFACE_KEYS[i] for i, s in enumerate(PRIMARY_COMMAND_STEPS)}
LIVE_API_BY_STEP = {'cpd_estimate': 'apply_multi_phase_estimate_day', 'cpd_phase': 'apply_combat_phase_close_day', 'cpd_joint': 'apply_combat_agent_joint_close_day', 'cpd_fleet': 'apply_fleet_day_posture', 'cpd_close': 'apply_production_oob_close_day'}
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label="Combat/production depth primary audit · dead %d · %s"%(len(dead),"PASS" if ok else "FAIL")
    return {"action_ids":ids,"dead":dead,"dead_n":len(dead),"ok":ok,"summary":label,"plain":label,"empty":False}

def build_combat_production_depth_primary_command_product(*, province_id=1, live_ids=None):
    ...
    for i,step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        lab="combat_production_depth · %s · live %s · score %.2f"%(step,api,sc)
    ...
    label="Combat/production depth primary · majors %d/5 · dead %d · score %.2f · %s"%(majors_ok_n,dead_n,score,"PASS" if all_majors_ok else "PARTIAL")
    out={"score":score,"plain":label+"\n"+"\n".join(r["label"] for r in steps_out),"summary":label,"empty":False,"province_id":pid,"surface_keys":list(SURFACE_KEYS),"majors":list(SURFACE_KEYS),"majors_ok":majors_ok,"majors_ok_n":majors_ok_n,"all_majors_ok":all_majors_ok,"dead_n":dead_n,"dead_ok":bool(audit.get("ok")),"audit":audit,"steps":steps_out,"step_ids":list(PRIMARY_COMMAND_STEPS),"step_scores":scores,"live_api_by_step":dict(LIVE_API_BY_STEP),"primary_action_ids":list(PRIMARY_ACTION_IDS),"apply_queue":apply_queue,"integration":["combat_production_depth_primary_command_product"]}
    if "combat_production_depth" == "n3_preflight":
        out["netcode_ready"]=False; out["not_full_n3"]=True
    return out

def apply_combat_production_depth_primary_command_step(step, province_id=1, *, runtime=None):
    ...
    product=build_combat_production_depth_primary_command_product(province_id=province_id)
