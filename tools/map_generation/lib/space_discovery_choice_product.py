"""Space discovery choice primary product."""
from __future__ import annotations
from pathlib import Path
from typing import Any, Dict

ROOT = Path(__file__).resolve().parents[3]
SURFACE_KEYS = ('sdc_primary_catalog', 'sdc_primary_discover', 'sdc_primary_list', 'sdc_primary_resolve', 'sdc_primary_close')
PRIMARY_COMMAND_STEPS = ('sdc_catalog', 'sdc_discover', 'sdc_list', 'sdc_resolve', 'sdc_close')
_STEP_MAJOR = {s: SURFACE_KEYS[i] for i, s in enumerate(PRIMARY_COMMAND_STEPS)}
LIVE_API_BY_STEP = {'sdc_catalog': 'apply_space_discovery_choice_catalog_live', 'sdc_discover': 'apply_space_discovery_choice_discover_live', 'sdc_list': 'apply_space_discovery_choice_list_live', 'sdc_resolve': 'apply_space_discovery_choice_resolve_live', 'sdc_close': 'apply_space_discovery_choice_close_live'}
PRIMARY_ACTION_IDS = tuple(LIVE_API_BY_STEP.values())
LIVE_PRIMARY_ACTION_IDS = frozenset(PRIMARY_ACTION_IDS)
HOOKS = [('scripts/space/SpaceLayerManager.gd', 'func list_discovery_choices'), ('scripts/space/SpaceLayerManager.gd', 'func resolve_discovery_choice')]
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": len(dead)==0 and len(ids)>=5,
            "summary": "Space discovery choice audit", "plain": "Space discovery choice audit", "empty": False}

def build_space_discovery_choice_primary_command_product(*, province_id: int = 1, live_ids=None) -> Dict[str, Any]:
    ...
    return {"score": 0.93 if all_ok else 0.4, "plain": label, "summary": label, "empty": False,
        ...
        "integration": ["space_discovery_choice_product"]}
