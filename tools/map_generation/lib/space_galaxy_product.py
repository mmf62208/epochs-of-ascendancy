"""Space galaxy primary command product — pure + source hooks."""
...
SURFACE_KEYS = ('sgx_primary_catalog', 'sgx_primary_unlock', 'sgx_primary_claim', 'sgx_primary_board', 'sgx_primary_close')
PRIMARY_COMMAND_STEPS = ('sgx_catalog', 'sgx_unlock', 'sgx_claim', 'sgx_board', 'sgx_close')
_STEP_MAJOR = {s: SURFACE_KEYS[i] for i, s in enumerate(PRIMARY_COMMAND_STEPS)}
LIVE_API_BY_STEP = {'sgx_catalog': 'apply_space_galaxy_catalog_live', 'sgx_unlock': 'apply_space_galaxy_unlock_live', 'sgx_claim': 'apply_space_galaxy_claim_live', 'sgx_board': 'apply_space_galaxy_board_live', 'sgx_close': 'apply_space_galaxy_close_live'}
PRIMARY_ACTION_IDS = tuple(LIVE_API_BY_STEP.values())
LIVE_PRIMARY_ACTION_IDS = frozenset(PRIMARY_ACTION_IDS)


NODES = 8
def sparse_ok(n=NODES):
    return 2 <= n <= 12
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    return {
        ...
        "summary": "Space galaxy audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL"),
        "plain": "Space galaxy audit", "empty": False,
    }


def build_space_galaxy_primary_command_product(*, province_id: int = 1, live_ids=None) -> Dict[str, Any]:
    pid = max(1, int(province_id))
    logic_ok = bool(sparse_ok(8))
    mgr = (ROOT / "scripts/space/SpaceLayerManager.gd").read_text(encoding="utf-8")
    hooks_ok = all(h in mgr for h in ['func get_galaxy_board', 'func claim_galaxy_node', 'func unlock_galaxy_for_view'])
    ...
    return {
        ...
        "integration": ["space_galaxy_product"],
    }
