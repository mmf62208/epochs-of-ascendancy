"""Space rival survey primary product."""
from __future__ import annotations
from pathlib import Path
from typing import Any, Dict

ROOT = Path(__file__).resolve().parents[3]
SURFACE_KEYS = ('srs_primary_catalog', 'srs_primary_seed', 'srs_primary_tick', 'srs_primary_compete', 'srs_primary_close')
PRIMARY_COMMAND_STEPS = ('srs_catalog', 'srs_seed', 'srs_tick', 'srs_compete', 'srs_close')
_STEP_MAJOR = {s: SURFACE_KEYS[i] for i, s in enumerate(PRIMARY_COMMAND_STEPS)}
LIVE_API_BY_STEP = {'srs_catalog': 'apply_space_rival_survey_catalog_live', 'srs_seed': 'apply_space_rival_survey_seed_live', 'srs_tick': 'apply_space_rival_survey_tick_live', 'srs_compete': 'apply_space_rival_survey_compete_live', 'srs_close': 'apply_space_rival_survey_close_live'}
PRIMARY_ACTION_IDS = tuple(LIVE_API_BY_STEP.values())
LIVE_PRIMARY_ACTION_IDS = frozenset(PRIMARY_ACTION_IDS)
HOOKS = [('scripts/space/SpaceLayerManager.gd', 'func process_monthly_rival_surveys'), ('scripts/space/SpaceLayerManager.gd', 'func start_survey')]
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": len(dead)==0 and len(ids)>=5,
            "summary": "Space rival survey audit", "plain": "Space rival survey audit", "empty": False}

def build_space_rival_survey_primary_command_product(*, province_id: int = 1, live_ids=None) -> Dict[str, Any]:
    ...
    return {"score": 0.93 if all_ok else 0.4, "plain": label, "summary": label, "empty": False,
        ...
        "integration": ["space_rival_survey_product"]}
