"""HOI panel polish primary — residual catalog product.
LIVE_API = real GameData methods (not bare apply_focus).
"""
from __future__ import annotations
from typing import Any, Dict

SURFACE_KEYS = ('hoi_primary_compact', 'hoi_primary_chips', 'hoi_primary_inspector', 'hoi_primary_apply', 'hoi_primary_close')
PRIMARY_COMMAND_STEPS = ('hoi_compact', 'hoi_chips', 'hoi_inspector', 'hoi_apply', 'hoi_close')
_STEP_MAJOR = {s: SURFACE_KEYS[i] for i, s in enumerate(PRIMARY_COMMAND_STEPS)}
LIVE_API_BY_STEP = {'hoi_compact': 'apply_product_ux_compact_board', 'hoi_chips': 'apply_product_ux_top_chips', 'hoi_inspector': 'apply_inspector_decision_product', 'hoi_apply': 'apply_inspector_product_apply', 'hoi_close': 'apply_inspector_surface_close_day'}
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "HOI panel polish primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_hoi_panel_primary_command_product(*, province_id: int = 1, live_ids=None):
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "HoiPanel · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "HOI panel polish primary · majors %d/5 · dead %d · score %.2f · %s" % (majors_ok_n, dead_n, score, "PASS" if all_majors_ok else "PARTIAL")
    return {
        ...
        "apply_queue": apply_queue, "integration": ["hoi_panel_primary_command_product", "HoiPanel"],
    }


def apply_hoi_panel_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    ...
    product = build_hoi_panel_primary_command_product(province_id=province_id)
