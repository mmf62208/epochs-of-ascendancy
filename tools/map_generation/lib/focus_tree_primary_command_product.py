"""Focus tree primary package — Master Plan T2.
catalog → path → commit → product → close. LIVE_API = real GameData methods.
"""
from __future__ import annotations
from typing import Any, Dict, Optional

try:
    from focus_tree_content_product import build_focus_tree_content_product  # type: ignore
except Exception:  # pragma: no cover
    def build_focus_tree_content_product(*_a, **_k):  # type: ignore
        return {"score": 0.62, "empty": False}

SURFACE_KEYS = (
    "focus_primary_catalog",
    "focus_primary_path",
    "focus_primary_commit",
    "focus_primary_product",
    "focus_primary_close",
)
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "Focus tree primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_focus_tree_primary_command_product(*, province_id: int = 1, live_ids=None):
    pid = max(1, int(province_id))
    try:
        composed = build_focus_tree_content_product(province_id=pid)
    except TypeError:
        composed = build_focus_tree_content_product()
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "T2 · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "Focus tree primary · majors %d/5 · dead %d · score %.2f · %s" % (
    ...
    return {
        ...
        "integration": ["focus_tree_primary_command_product", "focus_tree_content_product", "T2", "focus"],
    }


def apply_focus_tree_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    s = str(step or "").strip().lower()
    aliases = {"catalog": "focus_catalog", "path": "focus_path", "commit": "focus_commit",
               "product": "focus_product", "close": "focus_close"}
    s = aliases.get(s, s)
    if s not in PRIMARY_COMMAND_STEPS:
        s = PRIMARY_COMMAND_STEPS[0]
    product = build_focus_tree_primary_command_product(province_id=province_id)
...
def close_focus_tree_primary_command_package(province_id=1):
    rt = {"applied": []}
    steps = [apply_focus_tree_primary_command_step(s, province_id, runtime=rt) for s in PRIMARY_COMMAND_STEPS]
    product = build_focus_tree_primary_command_product(province_id=province_id)
    ...
    return {"ok": ok, "live": True, "product": product, "steps": steps, "applied_n": len(rt["applied"]),
            "dead_n": int(product.get("dead_n") or 0), "summary": "Focus tree close %s" % ("PASS" if ok else "FAIL"),
            "plain": "Focus tree close", "empty": False}


def focus_tree_primary_command_integrity():
    p = build_focus_tree_primary_command_product()
    c = close_focus_tree_primary_command_package()
    apis = list(LIVE_API_BY_STEP.values())
    no_focus = all(a != "apply_focus" and "apply_focus" not in a.replace("apply_focus_tree", "TREE") for a in apis)
    # apply_focus_tree_* is OK; bare apply_focus is not
    no_bare = all(not a.endswith("apply_focus") and a != "apply_focus" for a in apis)
    ok = (not p.get("empty") and int(p.get("dead_n", 1)) == 0 and bool(p.get("all_majors_ok"))
          and bool(c.get("ok")) and no_bare)
    return {"ok": ok, "no_focus": no_bare, "summary": "Focus tree integrity %s" % ("PASS" if ok else "FAIL"), "empty": False}
