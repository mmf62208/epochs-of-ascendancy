"""Play-session campaign primary package — Master Plan residual.
product → brief → execute → resolve → close.
...
def _compose(pid):
    try:
        return build_play_session_campaign_product(province_id=pid)
    except TypeError:
        try:
            return build_play_session_campaign_product()
        except Exception:
            return {"score": 0.7, "empty": False}
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "PSC primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_play_session_primary_command_product(*, province_id: int = 1, live_ids=None):
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "PSC · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "PSC primary · majors %d/5 · dead %d · score %.2f · %s" % (
    ...
    return {
        ...
        "integration": ["play_session_primary_command_product", "play_session_campaign_product", "PSC"],
    }


def apply_play_session_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    ...
    product = build_play_session_primary_command_product(province_id=province_id)
...
def close_play_session_primary_command_package(province_id=1):
    rt = {"applied": []}
    steps = [apply_play_session_primary_command_step(s, province_id, runtime=rt) for s in PRIMARY_COMMAND_STEPS]
    product = build_play_session_primary_command_product(province_id=province_id)
    ...
    return {"ok": ok, "live": True, "product": product, "steps": steps, "applied_n": len(rt["applied"]),
            "dead_n": int(product.get("dead_n") or 0), "summary": "PSC close %s" % ("PASS" if ok else "FAIL"),
            "plain": "PSC close", "empty": False}


def play_session_primary_command_integrity():
    p = build_play_session_primary_command_product()
    c = close_play_session_primary_command_package()
    ...
    return {"ok": ok, "no_focus": no_focus, "summary": "PSC integrity %s" % ("PASS" if ok else "FAIL"), "empty": False}
