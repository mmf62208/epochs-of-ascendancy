"""Q1 RC checklist primary.
LIVE_API = real GameData methods (not bare apply_focus).
"""
from __future__ import annotations
SURFACE_KEYS = ('q1r_primary_balance', 'q1r_primary_sample', 'q1r_primary_prove100', 'q1r_primary_variance', 'q1r_primary_close')
PRIMARY_COMMAND_STEPS = ('q1r_balance', 'q1r_sample', 'q1r_prove100', 'q1r_variance', 'q1r_close')
_STEP_MAJOR = {s: SURFACE_KEYS[i] for i, s in enumerate(PRIMARY_COMMAND_STEPS)}
LIVE_API_BY_STEP = {'q1r_balance': 'apply_balance_combat_supply_product', 'q1r_sample': 'apply_balance_live_sample', 'q1r_prove100': 'apply_medium_honesty_prove_100d', 'q1r_variance': 'apply_balance_variance_close', 'q1r_close': 'apply_balance_combat_supply_close_day'}
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label="Q1 RC checklist primary audit · dead %d · %s"%(len(dead),"PASS" if ok else "FAIL")
    return {"action_ids":ids,"dead":dead,"dead_n":len(dead),"ok":ok,"summary":label,"plain":label,"empty":False}

def build_q1_rc_checklist_primary_command_product(*, province_id=1, live_ids=None):
    ...
    for i,step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        lab="q1_rc_checklist · %s · live %s · score %.2f"%(step,api,sc)
...
def apply_q1_rc_checklist_primary_command_step(step, province_id=1, *, runtime=None):
    ...
    product=build_q1_rc_checklist_primary_command_product(province_id=province_id)
