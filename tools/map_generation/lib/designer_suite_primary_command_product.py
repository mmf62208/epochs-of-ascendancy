"""Designer suite primary package — Master Plan D1.\ncatalog → pick → seed → product → close.
...
def _compose(pid):
    try:
        return build_designer_suite_product(province_id=pid)
    except TypeError:
        return build_designer_suite_product()
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "D1 primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_designer_suite_primary_command_product(*, province_id: int = 1, live_ids=None):
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "D1 · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "D1 primary · majors %d/5 · dead %d · score %.2f · %s" % (
    ...
    return {
        ...
        "integration": ['designer_suite_primary_command_product', 'designer_suite_product', 'D1', 'designer'],
    }


def apply_designer_suite_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    ...
    product = build_designer_suite_primary_command_product(province_id=province_id)
...
def close_designer_suite_primary_command_package(province_id=1):
    rt = {"applied": []}
    steps = [apply_designer_suite_primary_command_step(s, province_id, runtime=rt) for s in PRIMARY_COMMAND_STEPS]
    product = build_designer_suite_primary_command_product(province_id=province_id)
    ...
    return {"ok": ok, "live": True, "product": product, "steps": steps, "applied_n": len(rt["applied"]),
            "dead_n": int(product.get("dead_n") or 0), "summary": "D1 close %s" % ("PASS" if ok else "FAIL"),
            "plain": "D1 close", "empty": False}


def designer_suite_primary_command_integrity():
    p = build_designer_suite_primary_command_product()
    c = close_designer_suite_primary_command_package()
    ...
    return {"ok": ok, "no_focus": no_focus, "summary": "D1 integrity %s" % ("PASS" if ok else "FAIL"), "empty": False}
