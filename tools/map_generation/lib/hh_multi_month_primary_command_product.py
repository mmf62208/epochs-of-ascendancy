"""HH multi-month agenda primary package — Master Plan L residual (beyond L1 strip).\nproduct → commit → counterplay → sequence → close.
...
def _compose(pid):
    try:
        return build_hh_multi_month_agenda_product(province_id=pid)
    except TypeError:
        return build_hh_multi_month_agenda_product()


SURFACE_KEYS = (
    "hh_primary_product",
    "hh_primary_commit",
    "hh_primary_counterplay",
    "hh_primary_sequence",
    "hh_primary_close",
)
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "HH primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_hh_multi_month_primary_command_product(*, province_id: int = 1, live_ids=None):
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "HH · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "HH primary · majors %d/5 · dead %d · score %.2f · %s" % (
    ...
    return {
        ...
        "integration": ['hh_multi_month_primary_command_product', 'hh_multi_month_agenda_product', 'L1', 'HH'],
    }


def apply_hh_multi_month_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    ...
    product = build_hh_multi_month_primary_command_product(province_id=province_id)
...
def close_hh_multi_month_primary_command_package(province_id=1):
    rt = {"applied": []}
    steps = [apply_hh_multi_month_primary_command_step(s, province_id, runtime=rt) for s in PRIMARY_COMMAND_STEPS]
    product = build_hh_multi_month_primary_command_product(province_id=province_id)
    ...
    return {"ok": ok, "live": True, "product": product, "steps": steps, "applied_n": len(rt["applied"]),
            "dead_n": int(product.get("dead_n") or 0), "summary": "HH close %s" % ("PASS" if ok else "FAIL"),
            "plain": "HH close", "empty": False}


def hh_multi_month_primary_command_integrity():
    p = build_hh_multi_month_primary_command_product()
    c = close_hh_multi_month_primary_command_package()
    ...
    return {"ok": ok, "no_focus": no_focus, "summary": "HH integrity %s" % ("PASS" if ok else "FAIL"), "empty": False}
