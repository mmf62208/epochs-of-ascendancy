"""Autosave session primary package — Master Plan S2.\nbrowser → autosave → resume → checkpoint → close.
...
def _compose(pid):
    try:
        return build_save_browser_campaign_product(province_id=pid)
    except TypeError:
        try:
            return build_save_browser_campaign_product()
        except Exception:
            return {"score": 0.6, "empty": False}
...
LIVE_API_BY_STEP = {
    "as_browser": "apply_save_browser_campaign_product",
    "as_autosave": "apply_autosave_session_ops_day",
    "as_resume": "apply_save_browser_resume",
    "as_checkpoint": "apply_save_browser_checkpoint",
    "as_close": "apply_save_session_close_day",
}
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "S2 primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_autosave_session_primary_command_product(*, province_id: int = 1, live_ids=None):
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "S2 · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "S2 primary · majors %d/5 · dead %d · score %.2f · %s" % (
    ...
    return {
        ...
        "integration": ['autosave_session_primary_command_product', 'save_browser_campaign_product', 'S2', 'autosave'],
    }


def apply_autosave_session_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    s = str(step or "").strip().lower()
    aliases = {}
    # short aliases from last segment
    for full in PRIMARY_COMMAND_STEPS:
        short = full.split("_", 1)[-1]
        aliases[short] = full
        aliases[full] = full
    s = aliases.get(s, s)
    if s not in PRIMARY_COMMAND_STEPS:
        s = PRIMARY_COMMAND_STEPS[0]
    product = build_autosave_session_primary_command_product(province_id=province_id)
...
def close_autosave_session_primary_command_package(province_id=1):
    rt = {"applied": []}
    steps = [apply_autosave_session_primary_command_step(s, province_id, runtime=rt) for s in PRIMARY_COMMAND_STEPS]
    product = build_autosave_session_primary_command_product(province_id=province_id)
    ...
    return {"ok": ok, "live": True, "product": product, "steps": steps, "applied_n": len(rt["applied"]),
            "dead_n": int(product.get("dead_n") or 0), "summary": "S2 close %s" % ("PASS" if ok else "FAIL"),
            "plain": "S2 close", "empty": False}


def autosave_session_primary_command_integrity():
    p = build_autosave_session_primary_command_product()
    c = close_autosave_session_primary_command_package()
    ...
    return {"ok": ok, "no_focus": no_focus, "summary": "S2 integrity %s" % ("PASS" if ok else "FAIL"), "empty": False}
