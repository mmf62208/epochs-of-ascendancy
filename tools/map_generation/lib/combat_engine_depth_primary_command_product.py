"""Combat engine depth primary.
LIVE_API = real GameData methods (not bare apply_focus).
"""
from __future__ import annotations
SURFACE_KEYS = ('ced_primary_ops', 'ced_primary_campaign', 'ced_primary_surge', 'ced_primary_oob100', 'ced_primary_close')
PRIMARY_COMMAND_STEPS = ('ced_ops', 'ced_campaign', 'ced_surge', 'ced_oob100', 'ced_close')
_STEP_MAJOR = {s: SURFACE_KEYS[i] for i, s in enumerate(PRIMARY_COMMAND_STEPS)}
LIVE_API_BY_STEP = {'ced_ops': 'apply_combat_ops_day', 'ced_campaign': 'apply_combat_campaign_day', 'ced_surge': 'apply_production_surge_day', 'ced_oob100': 'apply_oob_horizon_100d', 'ced_close': 'apply_combat_agent_joint_close_day'}
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label="Combat engine depth primary audit · dead %d · %s"%(len(dead),"PASS" if ok else "FAIL")
    return {"action_ids":ids,"dead":dead,"dead_n":len(dead),"ok":ok,"summary":label,"plain":label,"empty":False}

def build_combat_engine_depth_primary_command_product(*, province_id=1, live_ids=None):
    ...
    for i,step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        lab="combat_engine_depth · %s · live %s · score %.2f"%(step,api,sc)
...
def apply_combat_engine_depth_primary_command_step(step, province_id=1, *, runtime=None):
    ...
    product=build_combat_engine_depth_primary_command_product(province_id=province_id)
