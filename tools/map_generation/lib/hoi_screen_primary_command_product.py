"""HOI screen depth primary.
LIVE_API = real GameData methods (not bare apply_focus).
"""
from __future__ import annotations
SURFACE_KEYS = ('hos_primary_hotkeys', 'hos_primary_polish', 'hos_primary_panel', 'hos_primary_collapse', 'hos_primary_close')
PRIMARY_COMMAND_STEPS = ('hos_hotkeys', 'hos_polish', 'hos_primary', 'hos_collapse', 'hos_close')
_STEP_MAJOR = {s: SURFACE_KEYS[i] for i, s in enumerate(PRIMARY_COMMAND_STEPS)}
LIVE_API_BY_STEP = {'hos_hotkeys': 'apply_product_ux_hotkeys', 'hos_polish': 'apply_product_ux_command_polish_product', 'hos_primary': 'apply_inspector_product_primary', 'hos_collapse': 'apply_inspector_product_collapse', 'hos_close': 'apply_product_ux_polish_close_day'}
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label="HOI screen depth primary audit · dead %d · %s"%(len(dead),"PASS" if ok else "FAIL")
    return {"action_ids":ids,"dead":dead,"dead_n":len(dead),"ok":ok,"summary":label,"plain":label,"empty":False}

def build_hoi_screen_primary_command_product(*, province_id=1, live_ids=None):
    ...
    for i,step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        lab="hoi_screen · %s · live %s · score %.2f"%(step,api,sc)
...
def apply_hoi_screen_primary_command_step(step, province_id=1, *, runtime=None):
    ...
    product=build_hoi_screen_primary_command_product(province_id=province_id)
