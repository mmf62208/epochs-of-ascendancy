"""Product UX polish primary package — Master Plan U2.
compact → chips → hotkeys → product → close.
...
try:
    from product_ux_command_polish_product import build_product_ux_command_polish_product  # type: ignore
except Exception:  # pragma: no cover
    def build_product_ux_command_polish_product(*_a, **_k):  # type: ignore
...
LIVE_API_BY_STEP = {
    "ux_compact": "apply_product_ux_compact_board",
    "ux_chips": "apply_product_ux_top_chips",
    "ux_hotkeys": "apply_product_ux_hotkeys",
    "ux_product": "apply_product_ux_command_polish_product",
    "ux_close": "apply_product_ux_polish_close_day",
}
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "Product UX primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_product_ux_primary_command_product(*, province_id: int = 1, live_ids=None):
    pid = max(1, int(province_id))
    try:
        composed = build_product_ux_command_polish_product(province_id=pid)
    except TypeError:
        composed = build_product_ux_command_polish_product()
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "U2 · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "Product UX primary · majors %d/5 · dead %d · score %.2f · %s" % (
    ...
    return {
        ...
        "integration": ["product_ux_primary_command_product", "product_ux_command_polish_product", "U2", "ux"],
    }


def apply_product_ux_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    s = str(step or "").strip().lower()
    aliases = {"compact": "ux_compact", "chips": "ux_chips", "hotkeys": "ux_hotkeys",
               "product": "ux_product", "close": "ux_close"}
    s = aliases.get(s, s)
    if s not in PRIMARY_COMMAND_STEPS:
        s = PRIMARY_COMMAND_STEPS[0]
    product = build_product_ux_primary_command_product(province_id=province_id)
...
def close_product_ux_primary_command_package(province_id=1):
    rt = {"applied": []}
    steps = [apply_product_ux_primary_command_step(s, province_id, runtime=rt) for s in PRIMARY_COMMAND_STEPS]
    product = build_product_ux_primary_command_product(province_id=province_id)
    ...
    return {"ok": ok, "live": True, "product": product, "steps": steps, "applied_n": len(rt["applied"]),
            "dead_n": int(product.get("dead_n") or 0), "summary": "Product UX close %s" % ("PASS" if ok else "FAIL"),
            "plain": "Product UX close", "empty": False}


def product_ux_primary_command_integrity():
    p = build_product_ux_primary_command_product()
    c = close_product_ux_primary_command_package()
    ...
    return {"ok": ok, "no_focus": no_focus, "summary": "Product UX integrity %s" % ("PASS" if ok else "FAIL"), "empty": False}
