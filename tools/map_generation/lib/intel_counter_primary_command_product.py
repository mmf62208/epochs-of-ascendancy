"""Intel counter primary package — Master Plan I3.\nops → sweep → depth → counterplay → close.
...
try:
    from agent_counterplay import plan_agent_counterplay  # type: ignore
except Exception:  # pragma: no cover
    def plan_agent_counterplay(*_a, **_k):  # type: ignore
        return {"score": 0.58, "empty": False}

def _compose(pid):
    try:
        r = plan_agent_counterplay(province_id=pid)
        if isinstance(r, dict):
            return r if "score" in r else {"score": 0.58, **r, "empty": False}
    except TypeError:
        try:
            return plan_agent_counterplay()
        except Exception:
            pass
    except Exception:
        pass
...
LIVE_API_BY_STEP = {
    "ict_ops": "apply_intel_network_counterintel",
    "ict_sweep": "apply_intel_counter_sweep",
    "ict_depth": "apply_intel_network_counterplay",
    "ict_counterplay": "apply_intel_cell_ops",
    "ict_close": "apply_intelligence_network_close_day",
}
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "I3 primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_intel_counter_primary_command_product(*, province_id: int = 1, live_ids=None):
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "I3 · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "I3 primary · majors %d/5 · dead %d · score %.2f · %s" % (
    ...
    return {
        ...
        "integration": ["intel_counter_primary_command_product", "I3"],
    }


def apply_intel_counter_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    ...
    product = build_intel_counter_primary_command_product(province_id=province_id)
...
def close_intel_counter_primary_command_package(province_id=1):
    rt = {"applied": []}
    steps = [apply_intel_counter_primary_command_step(s, province_id, runtime=rt) for s in PRIMARY_COMMAND_STEPS]
    product = build_intel_counter_primary_command_product(province_id=province_id)
    ...
    return {"ok": ok, "live": True, "product": product, "steps": steps, "applied_n": len(rt["applied"]),
            "dead_n": int(product.get("dead_n") or 0), "summary": "I3 close %s" % ("PASS" if ok else "FAIL"),
            "plain": "I3 close", "empty": False}


def intel_counter_primary_command_integrity():
    p = build_intel_counter_primary_command_product()
    c = close_intel_counter_primary_command_package()
    ...
    return {"ok": ok, "no_focus": no_focus, "summary": "I3 integrity %s" % ("PASS" if ok else "FAIL"), "empty": False}
