"""Inspector decision primary package — Master Plan HOI UI residual.\ndecision → primary → collapse → apply → close.
...
try:
    from inspector_decision_product import build_inspector_decision_product  # type: ignore
except Exception:  # pragma: no cover
    def build_inspector_decision_product(*_a, **_k):  # type: ignore
        return {"score": 0.7, "empty": False}

def _compose(pid):
    try:
        return build_inspector_decision_product(province_id=pid)
    except TypeError:
        return build_inspector_decision_product()


SURFACE_KEYS = (
    "ins_primary_decision",
    "ins_primary_primary",
    "ins_primary_collapse",
    "ins_primary_apply",
    "ins_primary_close",
)
...
LIVE_API_BY_STEP = {
    "ins_decision": "apply_inspector_decision_product",
    "ins_primary": "apply_inspector_product_primary",
    "ins_collapse": "apply_inspector_product_collapse",
    "ins_apply": "apply_inspector_product_apply",
    "ins_close": "apply_inspector_surface_close_day",
}
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "INS primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_inspector_decision_primary_command_product(*, province_id: int = 1, live_ids=None):
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "INS · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "INS primary · majors %d/5 · dead %d · score %.2f · %s" % (
    ...
    return {
        ...
        "integration": ['inspector_decision_primary_command_product', 'inspector_decision_product', 'UI'],
    }


def apply_inspector_decision_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    ...
    product = build_inspector_decision_primary_command_product(province_id=province_id)
...
def close_inspector_decision_primary_command_package(province_id=1):
    rt = {"applied": []}
    steps = [apply_inspector_decision_primary_command_step(s, province_id, runtime=rt) for s in PRIMARY_COMMAND_STEPS]
    product = build_inspector_decision_primary_command_product(province_id=province_id)
    ...
    return {"ok": ok, "live": True, "product": product, "steps": steps, "applied_n": len(rt["applied"]),
            "dead_n": int(product.get("dead_n") or 0), "summary": "INS close %s" % ("PASS" if ok else "FAIL"),
            "plain": "INS close", "empty": False}


def inspector_decision_primary_command_integrity():
    p = build_inspector_decision_primary_command_product()
    c = close_inspector_decision_primary_command_package()
    ...
    return {"ok": ok, "no_focus": no_focus, "summary": "INS integrity %s" % ("PASS" if ok else "FAIL"), "empty": False}
