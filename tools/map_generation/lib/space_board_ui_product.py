"""Space board UI primary command product."""
from __future__ import annotations
from pathlib import Path
from typing import Any, Dict

ROOT = Path(__file__).resolve().parents[3]
SURFACE_KEYS = ('sbu_primary_catalog', 'sbu_primary_board', 'sbu_primary_layer', 'sbu_primary_strips', 'sbu_primary_close')
PRIMARY_COMMAND_STEPS = ('sbu_catalog', 'sbu_board', 'sbu_layer', 'sbu_strips', 'sbu_close')
_STEP_MAJOR = {s: SURFACE_KEYS[i] for i, s in enumerate(PRIMARY_COMMAND_STEPS)}
LIVE_API_BY_STEP = {'sbu_catalog': 'apply_space_board_ui_catalog_live', 'sbu_board': 'apply_space_board_ui_board_live', 'sbu_layer': 'apply_space_board_ui_layer_live', 'sbu_strips': 'apply_space_board_ui_strips_live', 'sbu_close': 'apply_space_board_ui_close_live'}
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok,
            "summary": "Space board UI audit", "plain": "Space board UI audit", "empty": False}


def build_space_board_ui_primary_command_product(*, province_id: int = 1, live_ids=None) -> Dict[str, Any]:
    pid = max(1, int(province_id))
    mgr = (ROOT / "scripts/space/SpaceLayerManager.gd").read_text(encoding="utf-8")
    hooks_ok = all(h in mgr for h in ['func build_space_layer_ui_board', 'func set_view_layer', 'func get_layer_fog_board'])
    ui = (ROOT / "scripts/ui/SpaceLayerBoardView.gd").read_text(encoding="utf-8")
    hooks_ok = hooks_ok and "func build_space_layer_ui_board" in mgr and "class_name SpaceLayerBoardView" in ui
    ...
    return {
        ...
        "apply_queue": apply_queue, "integration": ["space_board_ui_product"],
    }
