"""Naval multi-phase campaign primary package — Master Plan residual.
product → posture → escort → strike → close.
...
def _compose(pid):
    try:
        return build_naval_multi_phase_campaign_product(province_id=pid)
    except TypeError:
        try:
            return build_naval_multi_phase_campaign_product()
        except Exception:
            return {"score": 0.55, "empty": False}
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "NMP primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_naval_multi_phase_primary_command_product(*, province_id: int = 1, live_ids=None):
    pid = max(1, int(province_id))
    try:
        composed = _compose(pid)
    except Exception:
        composed = {"score": 0.55, "empty": False}
    base = _floor(float(composed.get("score") or 0.55))
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "NMP · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "NMP primary · majors %d/5 · dead %d · score %.2f · %s" % (
    ...
    return {
        ...
        "integration": ["naval_multi_phase_primary_command_product", "naval_multi_phase_campaign_product", "NMP"],
    }


def apply_naval_multi_phase_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    ...
    product = build_naval_multi_phase_primary_command_product(province_id=province_id)
...
def close_naval_multi_phase_primary_command_package(province_id=1):
    rt = {"applied": []}
    steps = [apply_naval_multi_phase_primary_command_step(s, province_id, runtime=rt) for s in PRIMARY_COMMAND_STEPS]
    product = build_naval_multi_phase_primary_command_product(province_id=province_id)
    ...
    return {"ok": ok, "live": True, "product": product, "steps": steps, "applied_n": len(rt["applied"]),
            "dead_n": int(product.get("dead_n") or 0), "summary": "NMP close %s" % ("PASS" if ok else "FAIL"),
            "plain": "NMP close", "empty": False}


def naval_multi_phase_primary_command_integrity():
    p = build_naval_multi_phase_primary_command_product()
    c = close_naval_multi_phase_primary_command_package()
    ...
    return {"ok": ok, "no_focus": no_focus, "summary": "NMP integrity %s" % ("PASS" if ok else "FAIL"), "empty": False}
