"""Hotseat session primary package — residual N multiplayer depth (not N3 netcode).
lobby → banner → rotate → lock_audit → close.
LIVE_API = real SessionPlayers / GameData methods.
"""
...
LIVE_API_BY_STEP = {
    "hsp_lobby": "apply_hotseat_session_lobby_live",
    "hsp_banner": "apply_hotseat_turn_banner_live",
    "hsp_rotate": "apply_hotseat_session_rotate_live",
    "hsp_lock": "apply_hotseat_session_lock_audit_live",
    "hsp_close": "apply_hotseat_session_close_live",
}
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "Hotseat session primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_hotseat_session_primary_command_product(*, province_id: int = 1, live_ids=None):
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "HSP · %s · live %s · score %.2f" % (step, api, sc)
        steps_out.append({
    ...
    label = "Hotseat session primary · majors %d/5 · dead %d · score %.2f · %s" % (
    ...
    return {
        ...
        "integration": ["hotseat_session_primary_command_product", "N1", "N2", "hotseat"],
        "not_netcode": True,
    }


def apply_hotseat_session_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    ...
    product = build_hotseat_session_primary_command_product(province_id=province_id)
