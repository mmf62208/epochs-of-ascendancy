"""Focus war-path primary package — Master Plan residual.
product → pick → path → commit → close.
...
def _compose(pid):
    try:
        return build_focus_war_path_product(province_id=pid)
    except TypeError:
        try:
            return build_focus_war_path_product()
        except Exception:
            return {"score": 0.65, "empty": False}
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "FWP primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_focus_war_path_primary_command_product(*, province_id: int = 1, live_ids=None):
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "FWP · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "FWP primary · majors %d/5 · dead %d · score %.2f · %s" % (
    ...
    return {
        ...
        "integration": ["focus_war_path_primary_command_product", "focus_war_path_product", "FWP"],
    }


def apply_focus_war_path_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    s = str(step or "").strip().lower()
    aliases = {}
    for full in PRIMARY_COMMAND_STEPS:
        aliases[full] = full
        aliases[full.split("_", 1)[-1]] = full
    aliases["war_path"] = "fwp_path"
    aliases["path_step"] = "fwp_path"
    s = aliases.get(s, s)
    if s not in PRIMARY_COMMAND_STEPS:
        s = PRIMARY_COMMAND_STEPS[0]
    product = build_focus_war_path_primary_command_product(province_id=province_id)
...
def close_focus_war_path_primary_command_package(province_id=1):
    rt = {"applied": []}
    steps = [apply_focus_war_path_primary_command_step(s, province_id, runtime=rt) for s in PRIMARY_COMMAND_STEPS]
    product = build_focus_war_path_primary_command_product(province_id=province_id)
    ...
    return {"ok": ok, "live": True, "product": product, "steps": steps, "applied_n": len(rt["applied"]),
            "dead_n": int(product.get("dead_n") or 0), "summary": "FWP close %s" % ("PASS" if ok else "FAIL"),
            "plain": "FWP close", "empty": False}


def focus_war_path_primary_command_integrity():
    p = build_focus_war_path_primary_command_product()
    c = close_focus_war_path_primary_command_package()
    ...
    return {"ok": ok, "no_focus": no_focus, "summary": "FWP integrity %s" % ("PASS" if ok else "FAIL"), "empty": False}
