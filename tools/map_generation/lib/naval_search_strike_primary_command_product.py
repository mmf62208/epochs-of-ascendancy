"""Naval search/strike primary package — Master Plan residual (naval theater).
product → patrol → asw_escort → strike → close.
...
try:
    from naval_search_strike_product import build_naval_search_strike_product  # type: ignore
except Exception:  # pragma: no cover
    def build_naval_search_strike_product(*_a, **_k):  # type: ignore
        return {"score": 0.6, "empty": False}


def _compose(pid):
    try:
        return build_naval_search_strike_product(province_id=pid)
    except TypeError:
        try:
            return build_naval_search_strike_product()
        except Exception:
            return {"score": 0.6, "empty": False}
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "NSS primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_naval_search_strike_primary_command_product(*, province_id: int = 1, live_ids=None):
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "NSS · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "NSS primary · majors %d/5 · dead %d · score %.2f · %s" % (
    ...
    return {
        ...
        "integration": ["naval_search_strike_primary_command_product", "naval_search_strike_product", "naval", "NSS"],
    }


def apply_naval_search_strike_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    s = str(step or "").strip().lower()
    aliases = {}
    for full in PRIMARY_COMMAND_STEPS:
        aliases[full] = full
        aliases[full.split("_", 1)[-1]] = full
    aliases["asw_escort"] = "nss_asw"
    aliases["escort"] = "nss_asw"
    aliases["carrier"] = "nss_strike"
    s = aliases.get(s, s)
    if s not in PRIMARY_COMMAND_STEPS:
        s = PRIMARY_COMMAND_STEPS[0]
    product = build_naval_search_strike_primary_command_product(province_id=province_id)
...
def close_naval_search_strike_primary_command_package(province_id=1):
    rt = {"applied": []}
    steps = [apply_naval_search_strike_primary_command_step(s, province_id, runtime=rt) for s in PRIMARY_COMMAND_STEPS]
    product = build_naval_search_strike_primary_command_product(province_id=province_id)
    ...
    return {"ok": ok, "live": True, "product": product, "steps": steps, "applied_n": len(rt["applied"]),
            "dead_n": int(product.get("dead_n") or 0), "summary": "NSS close %s" % ("PASS" if ok else "FAIL"),
            "plain": "NSS close", "empty": False}


def naval_search_strike_primary_command_integrity():
    p = build_naval_search_strike_primary_command_product()
    c = close_naval_search_strike_primary_command_package()
    ...
    return {"ok": ok, "no_focus": no_focus, "summary": "NSS integrity %s" % ("PASS" if ok else "FAIL"), "empty": False}
