"""Tech research campaign primary package — Master Plan residual.
product → catalog → priority → field → close.
...
def _compose(pid):
    try:
        return build_tech_research_campaign_product(province_id=pid)
    except TypeError:
        try:
            return build_tech_research_campaign_product()
        except Exception:
            return {"score": 0.8, "empty": False}
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "TRC primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_tech_research_primary_command_product(*, province_id: int = 1, live_ids=None):
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "TRC · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "TRC primary · majors %d/5 · dead %d · score %.2f · %s" % (
    ...
    return {
        ...
        "integration": ["tech_research_primary_command_product", "tech_research_campaign_product", "TRC"],
    }


def apply_tech_research_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    ...
    product = build_tech_research_primary_command_product(province_id=province_id)
...
def close_tech_research_primary_command_package(province_id=1):
    rt = {"applied": []}
    steps = [apply_tech_research_primary_command_step(s, province_id, runtime=rt) for s in PRIMARY_COMMAND_STEPS]
    product = build_tech_research_primary_command_product(province_id=province_id)
    ...
    return {"ok": ok, "live": True, "product": product, "steps": steps, "applied_n": len(rt["applied"]),
            "dead_n": int(product.get("dead_n") or 0), "summary": "TRC close %s" % ("PASS" if ok else "FAIL"),
            "plain": "TRC close", "empty": False}


def tech_research_primary_command_integrity():
    p = build_tech_research_primary_command_product()
    c = close_tech_research_primary_command_package()
    ...
    return {"ok": ok, "no_focus": no_focus, "summary": "TRC integrity %s" % ("PASS" if ok else "FAIL"), "empty": False}
