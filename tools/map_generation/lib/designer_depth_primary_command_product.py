"""Designer depth primary package — Master Plan D residual.\nmodule → board → stats → multi_domain → close.
...
def _compose(pid):
    try:
        return build_designer_module_editor_product(province_id=pid)
    except TypeError:
        return build_designer_module_editor_product()
...
LIVE_API_BY_STEP = {
    "dd_module": "apply_designer_module_editor_product",
    "dd_board": "apply_designer_module_board",
    "dd_stats": "apply_designer_stats_field_product",
    "dd_multi": "apply_designer_multi_domain_campaign_product",
    "dd_close": "apply_designer_multi_domain_campaign_close_day",
}
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "DD primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_designer_depth_primary_command_product(*, province_id: int = 1, live_ids=None):
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "DD · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "DD primary · majors %d/5 · dead %d · score %.2f · %s" % (
    ...
    return {
        ...
        "integration": ['designer_depth_primary_command_product', 'designer_module_editor_product', 'D'],
    }


def apply_designer_depth_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    ...
    product = build_designer_depth_primary_command_product(province_id=province_id)
...
def close_designer_depth_primary_command_package(province_id=1):
    rt = {"applied": []}
    steps = [apply_designer_depth_primary_command_step(s, province_id, runtime=rt) for s in PRIMARY_COMMAND_STEPS]
    product = build_designer_depth_primary_command_product(province_id=province_id)
    ...
    return {"ok": ok, "live": True, "product": product, "steps": steps, "applied_n": len(rt["applied"]),
            "dead_n": int(product.get("dead_n") or 0), "summary": "DD close %s" % ("PASS" if ok else "FAIL"),
            "plain": "DD close", "empty": False}


def designer_depth_primary_command_integrity():
    p = build_designer_depth_primary_command_product()
    c = close_designer_depth_primary_command_package()
    ...
    return {"ok": ok, "no_focus": no_focus, "summary": "DD integrity %s" % ("PASS" if ok else "FAIL"), "empty": False}
