"""Balance combat/supply primary package — Master Plan Q residual.\nproduct → estimate → sample → variance → close.
...
try:
    from balance_combat_supply_product import build_balance_combat_supply_product  # type: ignore
except Exception:  # pragma: no cover
    def build_balance_combat_supply_product(*_a, **_k):  # type: ignore
        return {"score": 0.65, "empty": False}

def _compose(pid):
    try:
        return build_balance_combat_supply_product(province_id=pid)
    except TypeError:
        return build_balance_combat_supply_product()


SURFACE_KEYS = (
    "bal_primary_product",
    "bal_primary_estimate",
    "bal_primary_sample",
    "bal_primary_variance",
    "bal_primary_close",
)
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "BAL primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_balance_combat_supply_primary_command_product(*, province_id: int = 1, live_ids=None):
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "BAL · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "BAL primary · majors %d/5 · dead %d · score %.2f · %s" % (
    ...
    return {
        ...
        "integration": ['balance_combat_supply_primary_command_product', 'balance_combat_supply_product', 'Q'],
    }


def apply_balance_combat_supply_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    ...
    product = build_balance_combat_supply_primary_command_product(province_id=province_id)
...
def close_balance_combat_supply_primary_command_package(province_id=1):
    rt = {"applied": []}
    steps = [apply_balance_combat_supply_primary_command_step(s, province_id, runtime=rt) for s in PRIMARY_COMMAND_STEPS]
    product = build_balance_combat_supply_primary_command_product(province_id=province_id)
    ...
    return {"ok": ok, "live": True, "product": product, "steps": steps, "applied_n": len(rt["applied"]),
            "dead_n": int(product.get("dead_n") or 0), "summary": "BAL close %s" % ("PASS" if ok else "FAIL"),
            "plain": "BAL close", "empty": False}


def balance_combat_supply_primary_command_integrity():
    p = build_balance_combat_supply_primary_command_product()
    c = close_balance_combat_supply_primary_command_package()
    ...
    return {"ok": ok, "no_focus": no_focus, "summary": "BAL integrity %s" % ("PASS" if ok else "FAIL"), "empty": False}
