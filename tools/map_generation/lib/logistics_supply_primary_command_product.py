"""Logistics supply theater primary package — Master Plan supply residual.\nproduct → route → sustain → readiness → close.
...
def _compose(pid):
    try:
        return build_logistics_supply_theater_product(province_id=pid)
    except TypeError:
        return build_logistics_supply_theater_product()
...
LIVE_API_BY_STEP = {
    "log_product": "apply_logistics_supply_theater_product",
    "log_route": "apply_logistics_supply_route",
    "log_sustain": "apply_logistics_supply_sustain",
    "log_readiness": "apply_logistics_supply_readiness",
    "log_close": "apply_logistics_supply_close_day",
}
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "LOG primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_logistics_supply_primary_command_product(*, province_id: int = 1, live_ids=None):
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "LOG · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "LOG primary · majors %d/5 · dead %d · score %.2f · %s" % (
    ...
    return {
        ...
        "integration": ['logistics_supply_primary_command_product', 'logistics_supply_theater_product', 'supply'],
    }


def apply_logistics_supply_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    ...
    product = build_logistics_supply_primary_command_product(province_id=province_id)
...
def close_logistics_supply_primary_command_package(province_id=1):
    rt = {"applied": []}
    steps = [apply_logistics_supply_primary_command_step(s, province_id, runtime=rt) for s in PRIMARY_COMMAND_STEPS]
    product = build_logistics_supply_primary_command_product(province_id=province_id)
    ...
    return {"ok": ok, "live": True, "product": product, "steps": steps, "applied_n": len(rt["applied"]),
            "dead_n": int(product.get("dead_n") or 0), "summary": "LOG close %s" % ("PASS" if ok else "FAIL"),
            "plain": "LOG close", "empty": False}


def logistics_supply_primary_command_integrity():
    p = build_logistics_supply_primary_command_product()
    c = close_logistics_supply_primary_command_package()
    ...
    return {"ok": ok, "no_focus": no_focus, "summary": "LOG integrity %s" % ("PASS" if ok else "FAIL"), "empty": False}
