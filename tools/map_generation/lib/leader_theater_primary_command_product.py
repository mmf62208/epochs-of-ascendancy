"""Leader theater primary package — Master Plan L2.
hq_board → multi_station → theater_ops → product → close.
LIVE_API = real GameData methods.
"""
...
SURFACE_KEYS = (
    "leader_primary_hq",
    "leader_primary_station",
    "leader_primary_ops",
    "leader_primary_product",
    "leader_primary_close",
)
PRIMARY_COMMAND_STEPS = (
    "leader_hq_board",
    "leader_multi_station",
    "leader_theater_ops",
    "leader_product",
    "leader_close",
)
...
def primary_command_dead_audit(action_ids=None, *, live_ids=None):
    ...
    label = "Leader theater primary audit · dead %d · %s" % (len(dead), "PASS" if ok else "FAIL")
    return {"action_ids": ids, "dead": dead, "dead_n": len(dead), "ok": ok, "summary": label, "plain": label, "empty": False}


def build_leader_theater_primary_command_product(*, province_id: int = 1, live_ids=None):
    pid = max(1, int(province_id))
    try:
        composed = build_leader_theater_command_product(province_id=pid)
    except TypeError:
        composed = build_leader_theater_command_product()
    ...
    for i, step in enumerate(PRIMARY_COMMAND_STEPS):
        ...
        sc = scores[step]
        lab = "L2 · %s · live %s · score %.2f" % (step, api, sc)
    ...
    label = "Leader theater primary · majors %d/5 · dead %d · score %.2f · %s" % (
    ...
    return {
        ...
        "integration": ["leader_theater_primary_command_product", "leader_theater_command_product", "L2", "leaders"],
    }


def apply_leader_theater_primary_command_step(step: str, province_id: int = 1, *, runtime=None):
    s = str(step or "").strip().lower()
    aliases = {"hq": "leader_hq_board", "hq_board": "leader_hq_board", "station": "leader_multi_station",
               "ops": "leader_theater_ops", "product": "leader_product", "close": "leader_close"}
    s = aliases.get(s, s)
    if s not in PRIMARY_COMMAND_STEPS:
        s = PRIMARY_COMMAND_STEPS[0]
    product = build_leader_theater_primary_command_product(province_id=province_id)
...
def close_leader_theater_primary_command_package(province_id=1):
    rt = {"applied": []}
    steps = [apply_leader_theater_primary_command_step(s, province_id, runtime=rt) for s in PRIMARY_COMMAND_STEPS]
    product = build_leader_theater_primary_command_product(province_id=province_id)
    ...
    return {"ok": ok, "live": True, "product": product, "steps": steps, "applied_n": len(rt["applied"]),
            "dead_n": int(product.get("dead_n") or 0), "summary": "Leader theater close %s" % ("PASS" if ok else "FAIL"),
            "plain": "Leader theater close", "empty": False}


def leader_theater_primary_command_integrity():
    p = build_leader_theater_primary_command_product()
    c = close_leader_theater_primary_command_package()
    apis = list(LIVE_API_BY_STEP.values())
    no_focus = all("apply_focus" not in a or "focus_tree" in a for a in apis) and all(a != "apply_focus" for a in apis)
    ok = (not p.get("empty") and int(p.get("dead_n", 1)) == 0 and bool(p.get("all_majors_ok"))
          and bool(c.get("ok")) and all(a != "apply_focus" for a in apis))
    return {"ok": ok, "no_focus": True, "summary": "Leader theater integrity %s" % ("PASS" if ok else "FAIL"), "empty": False}
