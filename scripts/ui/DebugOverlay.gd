...
func _build_ui() -> void:
	...
	# Space Designer (key future element once unlocked by orbital/space techs: Satellite, Station, Ship with modules for propulsion, weapons, life support, sensors, hab).
	# Unlocks with tech like orbital_shipyard or space_design_foundations (reuse space race techs/rule_flags). Similar to mech popup.
	var space_design_btn := Button.new()
	space_design_btn.text = "🛸 Space Designer (Sat/Station/Ship - unlocks with space techs; design modules for Expanse feel)"
	space_design_btn.pressed.connect(_show_space_designer_popup)
	harness_section.add_child(space_design_btn)
...
func _show_space_designer_popup() -> void:
	# Basic space designer popup (Sat/Station/Ship). Unlocked via space techs (orbital_shipyard, space_design etc). Allows "design" with module choices for Expanse feel.
