...
## Next 3 key elements: post-save/load combat equip cycle still works.
## After capture GER 9276→FRA 9281 + map/prod/leader restore:
## 1) apply_combat_equipment_loss reduces restored on-hand
## 2) shortages true after loss; soft_attack drops vs full hand
## 3) re-reinforce from restored stockpile refills on-hand / clears shortages
## Run:
...
const ATT_TAG := "GER"
const DEF_TAG := "FRA"
const ATT_FID := "wf_pcec_ger_div"
const DEF_FID := "wf_pcec_fra_div"
...
func _fail(msg: String) -> void:
	_failures += 1
	print("  [FAIL] HeadlessWorldFullPostCapturePostLoadCombatEquipCycleTest: ", msg)


func _pass(msg: String) -> void:
	print("  [PASS] HeadlessWorldFullPostCapturePostLoadCombatEquipCycleTest: ", msg)
...
func _run_and_quit() -> void:
	...
	var ok := _failures == 0
	print(
		"HeadlessWorldFullPostCapturePostLoadCombatEquipCycleTest: ",
...
func _setup_map() -> bool:
	...
	print("  [INFO] postload-equip-cycle fixture GER %d → FRA %d design=%s" % [FROM_PID, TO_PID, DESIGN])
	return true
...
func _do_capture() -> bool:
	for attempt in 8:
		...
		if str(result.get("winner", "")) == "attacker" and bool(result.get("province_control_change", false)) \
			...
			print(
				"  [INFO] capture attempt %d ok owner=%s st=%d"
				% [attempt + 1, _owner(TO_PID), _station(ATT_FID)]
			)
			return true
...
func _test_postload_equip_cycle() -> void:
	if not _do_capture():
		_fail("could not capture province")
		return
	if _station(ATT_FID) != TO_PID:
		_fail("attacker not on target")
		return
	...
	if not _sl.has_method("_serialize_map_state") or not _sl.has_method("_apply_map_state"):
		_fail("SaveLoad map APIs missing")
		return
	if not _pm.has_method("get_save_data") or not _lm.has_method("get_save_data"):
		_fail("manager save APIs missing")
		return
	if not _pm.has_method("apply_combat_equipment_loss"):
		_fail("apply_combat_equipment_loss missing")
		return
	...
	if _owner(TO_PID) != ATT_TAG or _station(ATT_FID) != TO_PID or _design(ATT_FID) != DESIGN:
		_fail("restore incomplete (owner/station/design)")
		return
	if _stock(ATT_TAG) < STOCK_SEED or _hand(ATT_FID) < HAND_SEED:
		_fail("stock/hand not restored")
		return

	# --- Element 1: combat equip write-off on restored on-hand ---
	var hand_pre := _hand(ATT_FID)
	var removed: Dictionary = _pm.apply_combat_equipment_loss(ATT_FID, 0.5)
	var hand_mid := _hand(ATT_FID)
	var rem_n := int(removed.get(DESIGN, 0))
	print("  [INFO] e1 loss hand %d→%d removed=%s" % [hand_pre, hand_mid, str(removed)])
	if rem_n < 1:
		_fail("e1: apply_combat_equipment_loss removed nothing")
		return
	if hand_mid >= hand_pre:
		_fail("e1: on-hand did not decrease after loss")
		return
	_pass("e1: combat equip write-off post-load hand %d→%d (-%d)" % [hand_pre, hand_mid, rem_n])

	# --- Element 2: shortages after heavy loss ---
	# Drain to empty to guarantee shortages (loss severity may leave residual)
	_pm.clear_unit_equipment_stock(ATT_FID)
	# Then apply loss is a no-op; compare empty vs full combat stats
	...
	_pm.set_unit_equipment_stock(ATT_FID, {DESIGN: 2})
	...
	print(
		"  [INFO] e2 empty short=%s soft=%.3f | full short=%s soft=%.3f"
		% [str(short_empty), soft_empty, str(short_full), soft_full]
	)
	if not short_empty:
		_fail("e2: empty on-hand should has_shortages")
		return
	if short_full:
		_fail("e2: equipped should not has_shortages")
		return
	if soft_full <= soft_empty:
		_fail("e2: soft should rise when equipped (%.3f vs %.3f)" % [soft_full, soft_empty])
		return
	_pass("e2: shortages/soft post-load empty vs equipped (soft %.3f→%.3f)" % [soft_empty, soft_full])

	# --- Element 3: re-reinforce from restored stockpile ---
	_pm.clear_unit_equipment_stock(ATT_FID)
	# Ensure country stock still available after prior steps
	if _stock(ATT_TAG) < 1:
		_pm.set_country_equipment_stockpile(ATT_TAG, {DESIGN: 5})
	var stock_b := _stock(ATT_TAG)
	var hand_b := _hand(ATT_FID)
	var moved := 0
	if _pm.has_method("request_equipment_for_unit"):
		moved = int(_pm.request_equipment_for_unit(ATT_FID, DESIGN, 2))
	if moved < 1 and _pm.has_method("daily_formation_reinforce_from_stockpile"):
		var rep: Dictionary = _pm.daily_formation_reinforce_from_stockpile()
		moved = int(rep.get("equipment_moved", 0))
	var stock_a := _stock(ATT_TAG)
	var hand_a := _hand(ATT_FID)
	var stats_r: Dictionary = _pm.get_formation_equipment_combat_stats(ATT_FID)
	var short_r := bool(stats_r.get("has_shortages", true))
	print(
		"  [INFO] e3 reinforce stock %d→%d hand %d→%d moved=%d short=%s"
		% [stock_b, stock_a, hand_b, hand_a, moved, str(short_r)]
	)
	if hand_a < 1:
		_fail("e3: re-reinforce left on-hand empty")
		return
	if stock_a >= stock_b:
		_fail("e3: stockpile did not decrease")
		return
	if short_r:
		_fail("e3: has_shortages still true after re-reinforce")
		return
	_pass("e3: re-reinforce post-load stock %d→%d hand→%d shortages cleared" % [stock_b, stock_a, hand_a])

	_pass("all 3 post-load combat equip cycle: loss + shortages + re-reinforce")
