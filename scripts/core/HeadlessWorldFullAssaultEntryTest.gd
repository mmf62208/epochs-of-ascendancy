...
## world_full land assault entry: can_assault + execute on real adjacency (GER→FRA).
...
## Real world_full land edge: GER 9276 adjacent FRA capital 9281.
const FROM_PID := 9276
const TO_PID := 9281
const ATT_TAG := "GER"
const DEF_TAG := "FRA"
const ATT_FID := "wf_assault_ger_div"
const DEF_FID := "wf_assault_fra_div"
...
func _fail(msg: String) -> void:
	_failures += 1
	print("  [FAIL] HeadlessWorldFullAssaultEntryTest: ", msg)


func _pass(msg: String) -> void:
	print("  [PASS] HeadlessWorldFullAssaultEntryTest: ", msg)


func _run_and_quit() -> void:
	_run()
	var ok := _failures == 0
	print("HeadlessWorldFullAssaultEntryTest: ", "PASS" if ok else "FAIL", " (failures=", _failures, ")")
	quit(0 if ok else 1)
...
func _run() -> void:
	...
	if _pm == null or _lm == null or _mm == null:
		...
		return
	if not _setup_world_full_border_map():
		return
...
func _setup_world_full_border_map() -> bool:
	...
	if not found and adj_sys.has_method("get_neighbors"):
		for n in adj_sys.call("get_neighbors", FROM_PID):
			if int(n) == TO_PID:
				...
				break
	if not found:
		_fail("fixture edge %d-%d not in world_full adjacency" % [FROM_PID, TO_PID])
		return false
	...
	from_p.set("name", "GER Border Fixture")
	...
	to_p.set("name", "FRA Capital Fixture")
