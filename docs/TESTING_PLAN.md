...
- Province click → scrollable InfoPanel; Close/Esc keep Europe framed (north-strip edge-pan must not unlock/fly to Greenland; no Home required) and keep fills; empty-area left-drag after 8px slop pans (drag up = camera north) even after Esc/Close and while paused — `_process` `_ensure_left_drag_armed_from_physical` → `_left_drag_should_pan` → `_activate` (last_mouse = press origin) → camera; 1st–3rd consecutive empty-area drags each pan; finished drag must not pick land or sea or coarse or capital-star snap (Area2D never pick on press; `_left_pick_allowed_on_release` + `_left_release_must_skip_pick` + `_left_live_slop_is_drag` on release; sticky slop + camera-move latch survive leftover `_begin` and leftover `_rearm`; 2nd/3rd empty-area drags re-arm pan-active after leftover hold / `genuine_new_press`; edge-pan while left-down counts as THIS drag; Home clears sticky; search LineEdit must take focus); idle/clean-map **Esc → Command Center** (`_handle_escape_key` → `TopInfoBar._on_menu_pressed`; leftover hidden MainMenu/unit card must not sticky-block; dismiss-then-idle unchanged); unit-card Open fight must open Maginot Attacker/Defender sheet (not tooltip-only); unit-card Fill%/TOE is a 15–16px SUCCESS/WARNING line above the clip fold (Speed/Armor/Men + last-3 combat log on tooltip); **first** End shows Tokyo + Beiping standalone gold stars + China label (no SOV red hulls over China/Mongolia, no RUS West)
