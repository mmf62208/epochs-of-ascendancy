...
- First-session **G** (GER Maginot, after Begin → Europe → Home): toast plus a readable cyan supply-route polyline (camera-space ~26 screen px, dark halo, `ProvinceContainers` host z 250, `highlight_supply_route_path`). Hang-safe: no corridor BFS on the G key frame. Not the map-mode Supply menu.
