...
- First-session **G** (GER Maginot, after Begin → Europe): toast plus a readable gold supply-route polyline (zoom-aware width, dark halo, `highlight_supply_route_path`). Hang-safe: no corridor BFS on the G key frame.
