...
- Begin GER → Europe Home: ≥1 GER land Division chip is **visibly painted** (`show_unit_counters_for_zoom` Home floor 0.24; `_sync_unit_counter_paint` after Home/Begin). World Shift+Home still culls. Still-click chip chrome/label (not only the 48px disk) opens the docked Fill%/TOE card — Home-band hit disk tracks counter scale (`_unit_counter_hit_radius_world`).
