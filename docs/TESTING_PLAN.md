...
- Province click → scrollable InfoPanel; Close/Esc keep Europe framed (live-camera lock + reassert, no Greenland jump, no Home required) and keep fills; left-drag pan must not pick (`_left_pan_committed`); End shows Tokyo + Beiping/CHI overlay star + China label
