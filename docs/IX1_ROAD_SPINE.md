...
Play on `d18cbae` then **HARD-failed the same Esc→CC / Begin wall** after that layer-only gate went green. Cause: live DisplayServer ≠ headless layer probe — MapRenderer `_input` armed map pick / chip / assault before GUI (same `gui_get_hovered_control` miss class as Search Go / 4x); Begin `pressed` is release-default so a swallowed release never fires; Esc listened for `keycode == KEY_ESCAPE` only (physical / `ui_cancel` no-op). Later polling / `window_input` / mouse-CC still saw **zero post-boot EOA_LIVE_RAW_*** because computerUse never activated the **main** game X11 client. Fix now: keep title `_input` + sticky-open + DisplayServer poll; **host the title UI in a transient exclusive `LivingTitleHost` Window** (separate X11 client at the panel pixels); hide that host on Esc so Command Center is visible; WM close of the host is Begin (does not quit); **do not** `FLAG_ALWAYS_ON_TOP` the main game window (Alt+Tab exit class). Layer 120/130 **kept**. Headless `HeadlessIx1LivingTitleEscBeginTest` fails if the native-host path is missing.
...
Play on `2bc8f19` then **hang FIXED** (RSS flat ~2.0 GB) but **spine construction FAIL** — press queued Köln `710417` (`STATE queued`, `PROGRESS IDM.start eta=35`, panel `Road spine 0% · ETA 35 days`) and the calendar ran **80 live days** at 1x/4x with **no further PROGRESS/STATE/COMPLETE**. Headless RoadSpineComplete/DayTick still 0→100% in 36 days. Gap: live `MapRenderer.button` stores the project on the **IDM autoload** `active_projects[710417]`; daily advance is `TimeManager.game_day_advanced` → `IDM.advance_daily_projects`. Windowed light-sim `advance_days` only **queues** `day_emit`; stay-alive `_drop_smoke_deferred_load` **clears that queue** (and `_flush_sim_events` early-returns) so the HUD calendar rolls while IDM never ticks. **FIX3:** `advance_days` light path calls `_tick_live_construction_on_calendar_day` (product, same IDM store). Toast timer uses WeakRef (no `Object → Object`). Spine chrome sits **below** Settle (no “Now building…” overlap). Guard `tools/eoa_ix1_spine_live_progress_guard.sh` + `HeadlessIx1RoadSpineLiveStayAliveTickTest` drive `advance_real_time` under stay-alive (must FAIL on `2bc8f19`). Hang toast-trim snapshot+`remove_child` **kept**. Do **not** score product Begin/Esc/4x/clock PASS. Road effects remain proposal-only.

Play on `c7f24242` / guard-instrumented `328a838` then **Search→Köln→Go + start LOGIC PASS** but the **real Play launch still hung** after `MapRenderer.button`: no frames, no toast, RSS **2.0→12.7 GB** (replay after import **1.3→15.3 GB**). Isolated last line `LeaderEventUI._trim_live_f5_toast_stack.iter0 n=3`. Cause: `_dismiss_toast` used `queue_free` only, so `while get_child_count() > 2` (and `> 4` in `show_toast`) never dropped children and spun on child 0 (~140 MB/s) once Search + `post_news` had filled the stack. Fix: snapshot + `remove_child` then `queue_free`; dismiss is idempotent. Play-launch `tools/eoa_play_f5_smoke_auto_begin.sh` + viewport mouse guard: frames **324** / sidecar RSS **2016→2017 MB flat 60 s** (max **2107 MB**); `toast_n=2` + **Building…**; trim **exit n=2 iters=1**. Three-state / CompleteTest / hatch / stay-alive / Search / SPINE_START **kept**. Do **not** score product Begin/Esc/4x/clock PASS. The smoke harness is **not** the product.

Play on `9ebd17f` then **Search→Köln→Go + start LOGIC PASS** but **UX + stay-alive FAIL** — no toast / Building…; clock frozen 14 Jan; identical frames; no `EOA_ZOOM_BEGIN/END`; kernel **OOM 9.9 GB** (not a crash / harness quit). Headless 5/5 PASS. Cause: windowed `_draw` of `Ix1SpinePreviewDraw` at absolute GIS cents (Köln ~4254,944; edges 3–8u) with antialiased `draw_line` plus a light RoadLayer/sites rebuild on start. Fix: hub-local preview, finite/min-step/max-seg caps, no AA, toast/Building… this frame then deferred panel/soft-pan, sync preview notify, no RoadLayer rebuild while queued/construction, xvfb `EOA_SMOKE_FRAME_GUARD` (60 s: frames **16488**, sidecar RSS **370→1317 MB** flat; `rss_mb=1314 PASS`). That isolated overlay guard did **not** hit `MapRenderer.button` / toast trim. Three-state / CompleteTest / hatch / stay-alive / Search / SPINE_START **kept**. Do **not** score product Begin/Esc/4x/clock PASS. The smoke harness is **not** the product.
