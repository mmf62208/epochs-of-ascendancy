...
| **GitHub** | `origin/main` @ `78e178f` (F5 UNIT ORDER QA handles module_unlock). Play branch `eoa/l1-war-loop-slice` matches. Do not force-push over June Cursor history. |
...
- **Beta playtest clock (machine):** `TimeManager.advance_living_playtest_days(5)` drives the **real F5 1x path** (`advance_days` + split `day_emit` / `day_ai` / `day_battles` flush, 15 events for 5 days). Light capture, hang-safe AAR + capture news history (next-hex is first adjacent enemy; toast UI skipped headless; deferred map refresh still skipped on the compact clock). Headless quit skips calendar autosave. Never `execute_province_assault` on that clock. Not beta-complete (human B0 still open; F5 graphical 5–10d notes still human).
