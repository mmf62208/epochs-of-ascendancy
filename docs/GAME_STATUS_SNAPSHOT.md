...
- **Beta playtest clock (machine):** `TimeManager.advance_living_playtest_days(5)` advances ≥5 calendar days and emits `game_day_advanced`. `LandBattleBubbleLayer._centroid_of` coerces pid to int (no TypedDictionary.has(String)). Light capture, never `execute_province_assault` on that clock. Not beta-complete (human B0 still open).
