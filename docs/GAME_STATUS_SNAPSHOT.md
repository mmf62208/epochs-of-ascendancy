...
- **Stockpile → TOE (playtest):** unit card shows Fill% + stock rifles/trucks; daily reinforce toasts `Stockpile → TOE · N units · N eq`. First-session docked card is **320×220**: `Fill NN% · TOE …` is its own 16px CYAN/SUCCESS/WARNING wrap label + thin Fill bar above the clip fold (Fill is not Strength%; unknown Fill is `Fill —%`). Land-chip still-click opens that card from `_input` (`_try_open_land_chip_from_input`) so a province glance tooltip cannot steal GER Division hits (tooltip tree IGNORE; not a map-pick blocker). Land still-click opens **player-tag land only** (closest player land; no foreign `best_any`). If the first pick is air_wing / fleet / space_wing, still-click re-picks player land in the hit disks and resolves player land stationed at the chrome province via `_collect_formations_at_province` before neighbor icons. Disk + **hex-only** province miss then binds the nearest visible player-land `DemoUnitIcon` within `max(hit_r, CHROME_SPILL_WORLD)` (≥320; Berlin-Home ↔ Maginot `710173` ~278u). Miss_pid does not prefer the capital star. Begin GER → Europe Home **paints** GER Division chips (`show_unit_counters_for_zoom` Home floor 0.24 — Home fit is still strategic-tier; `_sync_unit_counter_paint` after Home/Begin). Home-band pick disk covers the full painted plate+label AABB (`_unit_counter_hit_radius_world` live DemoUnitIcon half-diagonal, else `0.5 × sprite × scale × √2 + label_pad` — not half-plate / 48px-only) so chrome/label still-clicks open Fill%/TOE. Org/Str/Rdy, XP, Fuel, Planning, Entrenchment, Men, Width, Speed, Armor + last-3 combat log stay on tooltip. Product `unit_card_fill_toe_visibility_product` on `--quick`.
- **NEXT live fight (playtest):** any open land battle outranks first-session WarLoop copy (`Open fight`). Completing bars / player fights / train / fuel still outrank that. Unit-card **Open fight** fires on press (not release) and must open the Maginot Attacker/Defender sheet even after a pan latch — `_input` must not `set_input_as_handled` over the unit card. Sheet hides `UnitDetailPopup` so it is not covered. Search LineEdit/Go path unchanged.
...
- **Pan-from-drag (playtest):** left-drag pans after 8px slop — **including empty-area / sea / void** (no Area2D required). `_process` accumulates slop and `_left_drag_should_pan` → `_left_pan_active` → camera delta (drag up = camera north) even while paused. Close/Esc GIS lock must not snap an active empty-area drag; after Esc/Close **release**, a new slop-exceeded drag unlocks (`_unlock_close_camera_for_left_drag_pan`) while north-strip edge suppress stays. Same button-down as Close still does not slide. `_input` calls `_finish_close_click_guard_on_new_press` **before** leftover skip-pick early-return (340d9d8: three empty-area drags never panned). Pick is skipped for **this left-button-down** once slop exceeds 8px **or the camera moves while left is down** (hex, sea, coarse, capital-star snap — including Atlantic water in the Home-Europe left edge-pan strip that slides onto Iberia / Lisboa with slop <8px). `_left_release_must_skip_pick` is checked at Area2D **release** (never press — Rio Grande Rise sea jump-zoom), `_unhandled_input` pick, and coarse/title. Area2D also returns while left is still down if live slop ≥8px or skip is already latched (f942ac3 Tropical Atlantic Waters); release sites gate skip + `_left_live_slop_is_drag` only (no new arm helpers). Slop accumulates **before** `modal_blocks_map_nav` return; `_left_drag_should_pan` starts pan from physical left-down even if `_left_pan_armed` was cleared. Sticky slop + `_left_cam_moved_this_down` survive leftover `_begin(true)` (c4c44b8 leftover wipe picked Lisboa). `_left_map_pick_blocked()` is true while sticky slop ≥8px or the camera-move latch is set. Home clears sticky + latch (Berlin after Home). End/wheel/WASD must not drop `_left_btn_down` or wipe sticky/latch. Leftover hold is 16 process frames after `_end` only (not a mid-drag 400ms box). Idle `_process` must not clear skip during leftover hold (idle-clear let THIS release pick); after leftover hold expires, `_rearm_left_drag_for_next_press` drops pan-active only (skip/slop/cam latch survive leftover Area2D/unhandled — 51dc2a4 Rio Grande Rise). Idle `_allow` also unsticks leftover `_left_btn_down` on physical button-up so a swallowed `_end` (CC dimmer) cannot block Drag1–3 origin reset (47af97a empty-area no camera move; hover glance hidden while left-down + slop/pan/skip — MAR North). Leftover hold unsticks `_left_btn_down` without `_rearm`; idle-up `_begin(true)` is not first-line blocked by leftover btn-down; `_input` reseeds on idle-up `event.pressed` (Input singleton can stay stale after CC dimmer — 650c85c-cc Drag1–3 no-move). Closing/queued MainMenu must not `modal_blocks_map_nav`. After Drag1, leftover hold still blocks `genuine_new_press`; `_input` reseeds origin/`_last_mouse_pos` when `_left_button_was_up` was true so Drag2+3 slop-pan like Drag1 without clearing skip/cam (1f48f56 Drag2+3 no-move; not PR 24 `_begin` seed). `genuine_new_press` requires idle button-up and is not leftover-hold. Activate captures press origin *before* `_left_drag_should_pan` / `_note` can `_begin`-reset it, then seeds `_last_mouse_pos` from that origin so the first camera apply has a non-zero delta (a16ee8e Drag1–3 never moved). No PR 16 `_ensure_left_drag_armed_from_physical` / `_left_pick_allowed_on_release`. Genuine new physical press always resets origin even if leftover skip/cam flags remain. `_left_live_slop_is_drag` latches skip-pick on committed slop before `_note`/`_begin` can reset (cf95762 first empty-drag also picked Rio Grande Rise). Mid-gesture left-down + live slop ≥8px sets `_left_skip_next_pick` once via `_latch_left_skip_pick_from_live_slop` before `_note` (eed9b5f Drag2 Labrador Approaches West was `_unhandled_input` spatial `_select_province`, not Area2D / coarse / title; PR 22 latch kept). Click after button-up (Alicante) still picks. Search LineEdit clicks must not arm map pan. Command Center / modal: no map pan. No province-id special case. Chip / Ctrl / Shift clicks stay immediate. Product `smoke_pan_esc_product` on `--quick`. Esc dismiss-then-idle unchanged (1st Esc closes inspector, 2nd opens CC).
- **Home key (playtest):** Home / Shift+Home handled in `_input` so search/GUI focus cannot swallow recenter Europe / fit world.
- **End key (playtest):** **first** press frames Tokyo + Beiping `902487` (no blank red hulls over Mongolia/China, no RUS West). While Asia End, hide **all** RoW `900000–949999` SOV/RUS/sov-red fills (no AABB). Protected = owner CHI/MON/JAP/MAN/MEN + pids `902487`/`903995` only — never SOV/RUS by name. Dump leftover visible red/SOV fills including protected. Home restores RoW fills. Standalone Tokyo/Beiping stars unchanged. Leftover click skipped.
- **Settle titles (playtest):** inspector Settle button uses the province name (London / Devon), not `#id`. Create path is hidden with empty text — never `Settle This Province Now`. Product `settle_title_product` on `--quick`. Click nearer a capital star than the polygon hit picks the capital (ENG London 711414, not Devon 711467). Nested city capitals (Camden and City of London, District of Columbia, Roma) use a **zoom-aware gold-star snap** (star glyph is larger than the tiny NUTS/DC polygon at Europe zoom). Living OOB chip wins over a colocated star (plain left-click); star snap still applies when the click missed a unit. Chips sit offset from capital stars so both stay distinct.
- **Living unit counters (playtest):** DemoUnitIcon is a nation plate + NATO glyph + type letter + str% + designation + org/str/rdy bars. Political map does **not** stamp retrowave fort/port/airfield chips (those were the unclickable cyan hex blob on GER). Chip text is Node2D, not Control Labels (800 Labels froze F5). Pin-first when the chip is visible.
...
- **L1 G/WarLoop hang-class (playtest):** **G** key frame never BFS/preview_player_route (toast + deferred hop-capped land path, or click-to-show). Polyline via `highlight_supply_route_path` — camera-space cyan Line2D (~26 screen px) on follow-viewport `SupplyRouteHighlightLayer` (layer 10) so first-session Europe Home **G** reads over chips. **L-on** still no 3520 BFS. Shift+I stays toast-only. **I** never builds the flow overlay (flag + existing layer only; WarLoop-armed overlay used to deferred-hang the next frame). Plain **I** also dismisses leftover inspector / unit card / tooltip. Inspector **Close** / Esc (`_input` beats search focus) / unit-card Close hide the stack immediately and `gui_release_focus` so keys/clicks return. Chip/inspector cycles skip the 3520 supply-outline walk. `highlight_supply_corridor` still exists for tests.
...
| **Command Center** | **CanvasLayer overlay** · wordmark + menu chrome · **✕ / ESC / click dimmer** · **dismiss-then-idle Esc** (`MapRenderer._input` `_handle_escape_key`: 1st Esc closes settle/inspector; idle Esc → `TopInfoBar._on_menu_pressed` deferred so the opening Esc cannot `_force_close`; same-press re-entry `_esc_stack_frame`; leftover FileDialog exclusive / hidden MainMenu must not sticky-block; never `queue_free` MainMenu) · **Ctrl+S / Ctrl+L** save/load (no F5/F9 collision) · Help lists WarLoop path · `first_session_hotkeys_product` + `smoke_pan_esc_product` (`esc_dismiss_then_idle_cc`) |
...
| **Unit counters (pins)** | OOB/NATO map chips · **culled at world-fit strategic** (~0.14) · Europe Home band (~0.33–1.55) paints even when tier is still strategic (`show_unit_counters_for_zoom` floor 0.24) · operational+ full chips · **Shift+U** master toggle · nation plate + org/str bars · docked unit card · **pin-first pick when visible** (Home-band hit disk tracks counter scale · 48px/20 floor · gold chip · no inspector · `[` `]` stack) · inspector **Close restores input** · `map_unit_counter_lod_product` + `unit_centric_pick_product` + `unit_counter_chrome_product` |
...
| Ligurian Sea leftover south of Spain (Mike bar #3, second Med basin) | **FEED-8 theater proof** — reuse sea `950119` Ligurian basin ring onto Genoa gulf; Alboran `950128` / Tyrrhenian / Maginot / Flanders land untouched; see [`MAP_SEAS_COARSEN_FEED8_LIGURIAN.md`](MAP_SEAS_COARSEN_FEED8_LIGURIAN.md) · HOLD merge |
| SE England shire land cells not uniform (Mike bar #1, one theater) | **FEED-9 theater proof** — split Oxfordshire `711438` (parent kept on historic-core `original_centroid` piece); append `711523`+; no Greater London grow; see [`MAP_SE_ENGLAND_SHIRE_LAND_UNIFORMITY.md`](MAP_SE_ENGLAND_SHIRE_LAND_UNIFORMITY.md) · HOLD merge |
| Tyrrhenian Sea leftover south of Spain (Mike bar #3, third Med basin) | **FEED-10 theater proof** — reuse sea `950120` Tyrrhenian basin ring onto west-Italy / Corsica–Sardinia water; Ligurian `950119` / Alboran `950128` / SE England land untouched; see [`MAP_SEAS_COARSEN_FEED10_TYRRHENIAN.md`](MAP_SEAS_COARSEN_FEED10_TYRRHENIAN.md) · HOLD merge |
| Player↔map interconnect (Mike Layer 2) | **IX-1 Road Spine** — GER Rhineland `710416` Bonn–`710417` Köln–`710418` Leverkusen; **FIX3** live calendar ticks IDM (`_tick_live_construction_on_calendar_day`) so stay-alive dropping `day_emit` cannot leave spine at 0% while the HUD date rolls; toast WeakRef; chrome **below** Settle; live-progress guard = viewport mouse + `advance_real_time` (FAIL on `2bc8f19`); live Search **LineEdit + Go/Enter** (`pressed` / `text_submitted`) force-opens province inspector with **Build Road Spine** pinned on chrome next to Settle (and first facility Build row) — city-name + umlaut fold, search-rect click (not map-pan swallow), rebind after living title, miss toast, soft pan, inspector z above Garrison; Alt/infra empty-terrain backup; Esc releases Search focus then CC; inspector Build Road Spine → first-session Mandate grant (GER 1936 day-0 Mandate 0) → `build_road_connection` + RoadLayer + cheaper `get_movement_cost`; F5 day-tick + **live hour-clock FIX R2** (windowed DisplayServer is `is_live_f5_play_path`; leftover pick-block cannot swallow TopInfoBar 4x/pause — `_top_bar_owns_click` rect; Begin `mark_living_title_closed` so TestRunner will not re-pause; play-begin 4x gate FAILS if still paused at 00:00; live-equiv **past hour +6** and **past day +6**) so softpipe Begin→4x/Space leaves 1 Jan 00:00; **past-+6 wedge FIX** (6 Jan 20:00): skip live-F5 agent-network pulse + factory/presence day rebuild; player-cap reinforce; toast IGNORE + combat coalesce; Map Mode IGNORE+clip; UILayer 110 above Map Mode 20 / toasts 90; soak FAILS if still 6 Jan; **Esc/Begin FIX** (d53ee05 layers + d18cbae live input + **3d00182 Esc ×2 / poll**): living title 120 + CC 130 stay above that HUD; title owns `_input` + `_process` Input poll (Esc keycode/physical/key_label/`ui_cancel`, Begin PRESS + rect); sticky open-only so Play Esc ×2 cannot toggle-close CC; MapRenderer `_living_title_owns_click` so `_input` cannot swallow Begin or open chips (window-stay); **2a4ed6b Esc-delivery FIX**: computerUse Esc never reached Godot (zero `EOA_LIVE_ESC`) — keep poll/sticky-open + `Window.window_input`/focus/`ui_cancel`; ship **Command Center · Esc** + **Esc · Menu** chip; Begin dismisses title without Esc first; title-up TopInfoBar clicks pass through; **f9f249c zero-PTR FIX**: poll `DisplayServer.mouse_get_button_state()` (computerUse click-to-focus eats ButtonPress); `EOA_LIVE_RAW_PTR`/`EOA_LIVE_RAW_KEY`; Enter/Space/B Begin; play-like prove without windowactivate; **6573d01 Play F5 zero post-boot RAW**: computerUse never entered this X11 window (`EOA_LIVE_RAW_PTR`=1 `title.ready` only) — **LivingTitleHost** transient exclusive Window now hosts Begin/Esc so those pixels are their own X11 client; main window is not always-on-top (Alt+Tab exit); host hides on Esc so CC is visible; WM-close of the host is Begin (no quit); smoke-only `EOA_SMOKE_AUTO_BEGIN=1` / `tools/eoa_play_f5_smoke_auto_begin.sh` still dismisses via `handle_live_begin` so softpipe can proceed (not product Begin PASS until Play proves the host); **ae78507 hatch PASS / past-+6 FAIL**: clock stayed 1 Jan after 4x (same delivery hole); smoke-only `EOA_SMOKE_ADVANCE_PAST_PLUS6` (implied by AUTO_BEGIN unless 0) drives TopInfoBar 4x + `advance_real_time` past 7 Jan — not product clock PASS; **838487a live wedge FIX**: windowed smoke is **1 tick/frame** (`step_smoke_advance_chunk`) + deferred combat (never sync ×48); live log `past7=true` + `window_stay=1`; **8129648 catch-up FIX**: idle-starve pump so past7 lands before deferred grand visuals; **9625020 stay-alive FIX**: after past7 drop queued day_ai/battles, pause after ok, gate 252-icon flood, log `EOA_SMOKE_STAYALIVE` `no_quit=1` (Search window; not product clock PASS); **8f89145 Search chrome FIX**: host live Search LineEdit+Go on UILayer 110 with explicit TOP_LEFT pixels after stay-alive (was WorldMap UI 20 + TOP_RIGHT off-screen / 0-height under expanded Map Mode); Search Go + spine CTA pin kept; **002df244 silent-exit FIX**: spine start no longer snaps tactical 2.4 (`focus_province_by_id` soft + toast/Building… first); `EOA_ZOOM_BEGIN/END` flushed; light + capped RoadLayer rebuild; Köln progress/ETA + `EOA_SMOKE_SPINE_PROGRESS/COMPLETE`; corridor **queued → construction → built** (`EOA_SMOKE_SPINE_STATE`, `_draw` preview — zoom does not rebuild RoadLayer); `HeadlessIx1RoadSpineCompleteTest` start→complete + state order + Bonn–Köln–Leverkusen + Essen off-spine; harness `EOA_HARNESS_QUIT` reason-logged; headless `show_toast` no-op; **windowed hang/OOM FIX** after `9ebd17f` press (kernel OOM 9.9 GB, frames frozen): hub-local preview `_draw`, finite/min-step/max-seg caps, no AA, toast/Building… this frame then deferred panel/soft-pan, no RoadLayer rebuild while queued/construction, xvfb `EOA_SMOKE_FRAME_GUARD` 60s frames 16488 / sidecar RSS 370→1317 MB (`rss_mb=1314 PASS`) — that isolated overlay did not hit `MapRenderer.button` / toast trim; **c7f24242 / 328a838 live toast-trim hang FIX**: `LeaderEventUI._trim_live_f5_toast_stack` line 316 (`while get_child_count()>2` + `queue_free`) spun after `MapRenderer.button` (~140 MB/s; isolated `iter0 n=3`; Play RSS 2.0→12.7 GB / replay 1.3→15.3 GB); snapshot + `remove_child` then `queue_free`; Play launch `tools/eoa_play_f5_smoke_auto_begin.sh` + viewport mouse: frames 324 / sidecar 2016→2017 MB (max 2107) / toast_n=2 + Building… / trim exit n=2 iters=1 — smoke hatch/stay-alive/Search/SPINE_START kept, not product Begin/Esc/4x/clock PASS; later movement/export/supply/fuel numbers are STATUS proposal only; see [`IX1_ROAD_SPINE.md`](IX1_ROAD_SPINE.md) · HOLD merge |
