...
- **End key (playtest):** **first** press frames Tokyo + Beiping `902487` at operational zoom (no blank red hulls over Mongolia/China, no RUS West inspector). Hide SOV hulls `903528` / `903500` / `903491` plus any RoW SOV AABB that intersects CHI/MON; CHI/JAP/MON RoW z sits above SOV. Leftover click after the camera jump is skipped (`_left_skip_next_pick`; do not arm still-click). Standalone Polygon2D gold stars for Tokyo and Beiping sit off the unit chips. Pins a visible **China** nation label. Second press must not be required.
