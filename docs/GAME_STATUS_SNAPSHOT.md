...
- **Pan-from-drag (playtest):** left-drag pans after 8px slop. Pick is skipped for **this left-button-down** once slop exceeds 8px (hex, sea, coarse, **or zoom-aware capital-star snap**). A leftover `pressed=true` at the release screen pos is still THIS drag — `_begin(true)` only resets slop when the new press is ≥8px away (Berlin after Home). `_allow_left_pan_skip_to_die` must not clear slop (extra-frame die let leftover snap to Paris). No time-box. No province-id special case. Star-snap / `_select_province` gates on `_left_map_pick_blocked()` before resolve. Chip / Ctrl / Shift clicks stay immediate.
