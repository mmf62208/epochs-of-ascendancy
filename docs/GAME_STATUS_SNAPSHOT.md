...
- **Pan-from-drag (playtest):** left-drag pans after 8px slop. Pick is skipped for **this left-button-down** once slop exceeds 8px (`_left_gesture_dragged`) — no time-box, no idle-clear after `_end` (that dropped the skip before the release writer and opened Sarthe). `_note` / `_arm` must not start a slop-0 `_begin` after a pan. The skip clears only on the next genuine MouseButton press (`_begin_left_map_gesture(true)` — new button-down, slop 0). Every mouse writer gates on `_left_map_pick_blocked()`. A new press after button-up still picks. No province-id special case. Chip / Ctrl / Shift clicks stay immediate.
