...
- **Pan-from-drag (playtest):** left-drag pans after 8px slop. Pick is skipped from a **sticky press origin** (`_left_max_slop_sq`) plus a short post-pan block (`_left_block_pick_until_msec`) so a re-arm at the release hex cannot look like a still-click (Play: Atlantic 20px pan opened A Coruña). Click-without-slop still picks. Chip / Ctrl / Shift clicks stay immediate. MMB and right-drag unchanged. Map click releases search LineEdit focus so G is not typed as `Berling`.
