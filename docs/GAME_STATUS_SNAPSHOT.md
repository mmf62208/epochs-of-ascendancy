...
- **End key (playtest):** **first** press frames Tokyo + Beiping `902487` at operational zoom (no blank red hulls over Mongolia/China, no RUS West inspector). Hide SOV hulls `903528`/`903500`/`903491`/`903544`/`903553`/`903494`/`903498` plus KAZ east and any RoW SOV (or sov-red fill) whose AABB hits the Beiping–Tokyo hide rect. Standalone Polygon2D gold stars for Tokyo and Beiping stay off the chips. Pins **China**. Leftover click skipped. Second press must not be required.
