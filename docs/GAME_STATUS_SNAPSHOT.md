...
- **Pan-from-drag (playtest):** left-drag pans after 8px slop. Pick is skipped for **this left-button-down** once slop exceeds 8px (`_left_gesture_dragged` / `_left_slop_is_drag`). A leftover `pressed=true` at the release hex is still THIS drag — `_begin_left_map_gesture(true)` must not start a slop-0 gesture while slop/dragged/skip are set (Play: Orne / Sarthe / Atlantic sea / East Asia water). `_allow_left_pan_skip_to_die` clears slop only after a real button-up plus one extra process frame (not 400ms), so a later click `_begin(true)` can pick. No time-box. No province-id special case. Every mouse writer gates on `_left_map_pick_blocked()`. Chip / Ctrl / Shift clicks stay immediate.
