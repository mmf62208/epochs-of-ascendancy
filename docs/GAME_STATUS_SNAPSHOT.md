...
| **GitHub** | `origin/main` @ `7459c41` (B1–B7 + NEXT living player tag). Play branch `eoa/l1-war-loop-slice` matches. Do not force-push over June Cursor history. |
...
- **Beta playtest clock (machine):** `TimeManager.advance_living_playtest_days(5)` advances ≥5 calendar days on the Maginot living path with light capture (`_living_playtest_clock` → `is_interactive_light_sim`), never `execute_province_assault` on that clock. Split F5 `day_emit`/`day_ai`/`day_battles` unchanged. Not beta-complete (human B0 still open).
