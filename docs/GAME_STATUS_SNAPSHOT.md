...
- **Pan-from-drag (playtest):** left-drag pans after 8px slop. Pick is skipped for **this left-button-down** once slop exceeds 8px (hex, sea, coarse, capital-star snap). Leftover `pressed=true` while the physical button is **up** is still THIS drag — do not reset slop because the cursor moved from last `_end` (689d530 `away_from_release` picked Cornwall / East Asia coarse). `_end` no-ops while the button is still held. `_begin(true)` resets slop only on a real physical left-down (Berlin after Home). `_allow` must not clear slop. No time-box. No province-id special case. Every mouse writer gates on `_left_map_pick_blocked()`. Chip / Ctrl / Shift clicks stay immediate.
