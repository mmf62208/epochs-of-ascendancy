...
- **End key (playtest):** **first** press frames Tokyo + Beiping `902487` at operational zoom (no blank triangles, no RUS West inspector). Leftover click after the camera jump is skipped (`_left_skip_next_pick`; do not arm still-click / do not `ensure_world_navigation_ready` clamp-before-fit). Stamps both capital stars on a high-z `AsiaEndStarOverlay` and pins a visible **China** nation label. Second press must not be required.
