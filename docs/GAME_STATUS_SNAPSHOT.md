...
- **End key (playtest):** **first** press frames Tokyo + Beiping `902487` at operational zoom (no blank red triangles over Mongolia, no RUS West inspector). Do not pad −700 west of Beiping. Hide SOV sparse hulls `903528` / `903500` (those convex rings painted over N China / Mongolia). Leftover click after the camera jump is skipped (`_left_skip_next_pick`; do not arm still-click). Beiping overlay star sits opposite the garrison/division chip stack on a dark disc. Pins a visible **China** nation label. Second press must not be required.
