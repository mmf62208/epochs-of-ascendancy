...
- **L1 resolve hang-class (playtest):** F5 attacker-win defers `execute_province_assault` off the sim tick (`_deferred_resolve_attacker_win`). Encircle BFS skipped while interactive. Recovery/organize deferred when a fight resolves. Owner-change still pid-fill only (no 3520 border/mesh/labels). Overlay owner redraws skip on light sim. Time keeps advancing after `open land battles resolved`. LOD/mode still redraws frontiers.
