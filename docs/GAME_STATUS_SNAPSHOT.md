...
| **Command Center** | **CanvasLayer overlay** · wordmark + menu chrome · **✕ / ESC / click dimmer** · idle map **Esc → Command Center** (`MapRenderer._input` `_handle_escape_key` → `TopInfoBar._on_menu_pressed`; visible overlays only — leftover MainMenu/unit card must not sticky-block) · **Ctrl+S / Ctrl+L** save/load (no F5/F9 collision) · Help lists WarLoop path · `first_session_hotkeys_product` + `smoke_pan_esc_product` |
