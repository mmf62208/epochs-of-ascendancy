...
- **Pan-from-drag (playtest):** left-drag pans after 8px slop. Pick is skipped if `_left_pan_committed` **or** the camera moved from `_left_press_cam_pos` (flags can reset before `_unhandled_input`; capital-star snap must not select Oslo from a North Sea release). Snapshot resets only on the next press so a later click still picks. Chip / Ctrl / Shift clicks stay immediate. MMB and right-drag unchanged. Map click releases search LineEdit focus so G is not typed as `Berling`.
