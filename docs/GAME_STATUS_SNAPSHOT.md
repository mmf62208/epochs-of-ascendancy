...
**Date:** 2026-08-20 (L1 land loop + living units + organize + composition + **industry TOE stockpile reinforce** · `--quick` green · board ~3520 · M6 human-only open)  
...
- **L1 industry TOE stockpile (machine):** Factories consume steel/oil/rubber/etc. to produce **the same keys as designer TOE** (rifles/trucks/tanks/guns). Completes credit country equipment stockpile. `reinforce_unit_toe_from_stockpile` fills a share of the gap (field-vs-new); empty stock invents nothing. Headless: fill **0.20 → 0.28** after produce, then combat writes that stock (rifles −6 · trucks −3 · tanks −1), not a demo-only pack. Save round-trips stockpiles. Not HOI efficiency/conversion tables.
