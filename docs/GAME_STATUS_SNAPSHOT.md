...
**Date:** 2026-09-18 (human Maginot F5: click-order → occupy walk-in → **many FRA hexes** → calendar **12 Jan**; canvas pick; fill-tint hover; chip pid-hash; RSS sampler boot print; H = CAS not hills. Closeout: [`SESSION_NOTES/2026-09-18_playtest_closeout_plan.md`](SESSION_NOTES/2026-09-18_playtest_closeout_plan.md))  
...
| Next | Action |
|------|--------|
| **Human** | F5 **factory-toe** (`execute-plan/856bb385-pr-3-factory-toe-on-the-default-play-strip`), not stale `main`. Maginot chip → right-click Alsace → declare → 1× until break → walk-in. Watch RSS: boot prints `RSS sampler N KB (pause at 2500000)`. If 1× pauses, note `last=`. Ctrl+L loads last autosave. Paste for Grok: [`SESSION_NOTES/GROK_PASTE_2026-09-18.md`](SESSION_NOTES/GROK_PASTE_2026-09-18.md). M6 20d/60d still open. |
| **Machine** | Occupy-after-win: assault-hex only walks in; rear stays; no GER bounce. `HeadlessWorldAccurateUnitOrderLoopTest` RESULT=PASS. RSS trip still **unproven** on a long graphical 1× (human hit 5–18 GB with no pause before sampler-open fix). Soft 30fps still FAIL honest. |
| **GitHub** | **Play:** `origin/execute-plan/856bb385-pr-3-factory-toe-on-the-default-play-strip` (push this tree). `origin/main` @ `2d56b84` is **stale** vs play — do **not** merge until RSS pause proven + PLAYTEST §0b 11–15. Do not force-push over June Cursor history. |
...
- **L1 click-order (playtest):** selected chip **right-click** / **Ctrl+click** enemy **starts the fight** (`start_land_battle`) when the assault is legal — no extra box. Compact **will-gate** (Declare war / Negotiate access) only if `can_assault` fails. Own land still marches (adjacent; Shift-click plans the next hex — handled on mouse-up in `_input`). Unit card **Attack** is the same path. Minimap hidden; card clips. Stacks show peeking plates + **1/3**; **left-click the chip** or **[ ]** cycles (does not cancel). **Right-click the same hex** cancels that division's orders. Chip hit disk **32px / 16 wu**. Adjacent Rhine chips use a **pid-hash offset** (6 slots) so Maginot/Kusel plates do not share a pixel. Hover/pick uses **map canvas** transform (not `Camera2D.get_canvas_transform()`). Capital-star snap is **in-country only** (LUX cannot steal GER/FRA). Hover/select is **fill-tint**, not cyan NUTS Line2D. F5 `can_assault` uses cheap `land_combat_power` (no CombatResolver). Freeze notes: [`SESSION_NOTES/2026-09-16_maginot_combat_learnings.md`](SESSION_NOTES/2026-09-16_maginot_combat_learnings.md) · closeout [`SESSION_NOTES/2026-09-18_playtest_closeout_plan.md`](SESSION_NOTES/2026-09-18_playtest_closeout_plan.md).
- **2026-09-18 human Maginot (playtest):** factory-toe tree. Click-order → occupy walk-in → **many FRA hexes** → calendar **12 Jan 23:00**. Occupy **assault-hex only** (rear/marched-away not yanked). Resolve frame toast-only. **H** with air selected / right-click hex = CAS region (H is not 3520 elevation). Capture retasks player CAS onto the hex's air region; FRA air in Paris stays until Paris falls. Political map **no resource glyphs**; F9 painted goods. RSS boot prints sampler KB; trip at **2.5 GB** (statm via `FileAccess.open`, no awk fork). Long 1× still hit 5–18 GB **before** that sampler-open fix — **prove pause next sitting**. Autosave on quit 2026-09-18. Hex NUTS overlap / city-in-county / 13k is **parked**.
- **L1 fight card:** one briefing per battle (all `att_fids` + `att_pending_fids` JOINING). Title shows **NvM** (+joining). Second division ordered into an open fight **joins** (`try_reinforce_land_battle`) — does not rebuild the unit card. Map pulse = Likely/Tight/Bad bar (no %). Click red arrow shaft or a fighting chip opens **Fight · place**. Updates on join + daily.
...
- **Pan-from-drag (playtest):** left-drag pans after 8px slop. Pick is skipped for **this left-button-down** once slop exceeds 8px (hex, sea, coarse, capital-star snap). Sticky slop origin survives leftover `_begin(true)` (8dcec1f End/wheel set `_left_btn_down=false` then leftover reset picked Lisboa / East Asia coarse). `_left_map_pick_blocked()` is true while sticky slop ≥8px. Home clears sticky (Berlin after Home). End/wheel/WASD must not drop `_left_btn_down` or wipe sticky. Leftover hold is 16 process frames after `_end` only (not a mid-drag 400ms box). Search LineEdit clicks must not arm map pan. No province-id special case. Chip / Ctrl / Shift clicks stay immediate.
- **Home key (playtest):** Home / Shift+Home handled in `_input` so search/GUI focus cannot swallow recenter Europe / fit world.
- **End key (playtest):** frames Tokyo + Beiping `902487` at operational zoom, stamps both capital stars on a high-z `AsiaEndStarOverlay` (Beiping not buried under garrison/division chips), and pins a visible **China** nation label (survives viewport cull / label rebuild).
- **Settle titles (playtest):** inspector Settle button uses the province name (London / Devon), not `#id`. Click nearer a capital star than the polygon hit picks the capital (ENG London 711414, not Devon 711467). Nested city capitals (Camden and City of London, District of Columbia, Roma) use a **zoom-aware gold-star snap** (star glyph is larger than the tiny NUTS/DC polygon at Europe zoom). Living OOB chip wins over a colocated star (plain left-click); star snap still applies when the click missed a unit. Chips sit offset from capital stars so both stay distinct.
...
| **Map visual default** | **Clean political** — terrain underlay off · **no resource glyphs** (F9 resources mapmode only, painted goods icons) · continuous sea + slight zone tint · solid land fills · **HOI-style borders** (dark international only; internal province edges tactical-only — fixes NUTS spiderweb) · pink select outline kept · nation labels high-contrast |
