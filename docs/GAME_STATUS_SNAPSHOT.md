...
- **Beta playtest clock (machine):** `TimeManager.advance_living_playtest_days(5)` advances ≥5 calendar days, emits `game_day_advanced`, and ticks open land battles with sync light capture (no AAR next-hex, no capture news/refresh — those hung). Never `execute_province_assault` on that clock. Not beta-complete (human B0 still open).
