...
- **Pan-from-drag (playtest):** left-drag pans after 8px slop. Pick is skipped for **this left-button-down** once slop exceeds 8px (`_left_gesture_dragged`) — no time-box (400ms expired mid-drag and re-armed, opening Finistère). `_begin_left_map_gesture` must not reset that flag after `_end_left_button_down` (duplicate `pressed=true` at the release hex used to pick sea `#950208` / Asia coarse). Every mouse writer (`_unhandled_input` hex/star, `_on_province_input`, `_show_coarse_territory_info(focus_camera)`) gates on `_left_map_pick_blocked()`. A new press after button-up still picks. No province-id special case. Chip / Ctrl / Shift clicks stay immediate.
