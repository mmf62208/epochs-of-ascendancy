...
- **Pan-from-drag (playtest):** left-drag pans after 8px slop — **including empty-area / sea / void** (no Area2D required). `_process` accumulates slop and `_left_drag_should_pan` → `_left_pan_active` → camera delta (drag up = camera north) even while paused. Close/Esc GIS lock must not snap an active empty-area drag; after Esc/Close **release**, a new slop-exceeded drag unlocks (`_unlock_close_camera_for_left_drag_pan`) while north-strip edge suppress stays. Same button-down as Close still does not slide. `_input` calls `_finish_close_click_guard_on_new_press` **before** leftover skip-pick early-return (340d9d8: three empty-area drags never panned). Pick is skipped for **this left-button-down** once slop exceeds 8px **or the camera moves while left is down** (hex, sea, coarse, capital-star snap — including Atlantic water in the Home-Europe left edge-pan strip that slides onto Iberia / Lisboa with slop <8px). `_left_release_must_skip_pick` is checked at Area2D **release** (never press — Rio Grande Rise sea jump-zoom), `_unhandled_input` pick, and coarse/title. Slop accumulates **before** `modal_blocks_map_nav` return; `_left_drag_should_pan` starts pan from physical left-down even if `_left_pan_armed` was cleared. Sticky slop + `_left_cam_moved_this_down` survive leftover `_begin(true)` (c4c44b8 leftover wipe picked Lisboa). `_left_map_pick_blocked()` is true while sticky slop ≥8px or the camera-move latch is set. Home clears sticky + latch (Berlin after Home). End/wheel/WASD must not drop `_left_btn_down` or wipe sticky/latch. Leftover hold is 16 process frames after `_end` only (not a mid-drag 400ms box). Idle `_process` must not clear skip during leftover hold (idle-clear let THIS release pick); after leftover hold expires, `_rearm_left_drag_for_next_press` drops pan-active only (skip/slop/cam latch + `_left_release_screen` survive leftover Area2D/unhandled — 51dc2a4 Rio Grande Rise / e3fea3e Labrador). `_allow` must not set `_left_button_was_up` during leftover hold, and must unstick leftover `_left_btn_down` so Drag1 `_begin(true)` can seed origin. `genuine_new_press` requires idle button-up and is not leftover-hold or leftover-at-release. leftover-at-release reseeds origin and keeps skip (no sea pick). Activate seeds `_last_mouse_pos` from press origin (and adopts `_arm` last_mouse if origin was missing) so Drag1–3 each apply a camera delta. No PR 16 `_ensure_left_drag_armed_from_physical` / `_left_pick_allowed_on_release`. Genuine new physical press always resets origin even if leftover skip/cam flags remain. `_left_live_slop_is_drag` latches skip-pick on committed slop before `_note`/`_begin` can reset (cf95762 first empty-drag also picked Rio Grande Rise). Click after button-up (Alicante) still picks. Search LineEdit clicks must not arm map pan. Command Center / modal: no map pan. No province-id special case. Chip / Ctrl / Shift clicks stay immediate. Product `smoke_pan_esc_product` on `--quick`. Esc dismiss-then-idle unchanged (1st Esc closes inspector, 2nd opens CC).
