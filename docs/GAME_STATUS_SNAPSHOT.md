...
- **L1 click-order (playtest):** selected chip **right-click** / **Ctrl+click** enemy **starts the fight** (`start_land_battle`) when the assault is legal — no extra box. Compact **will-gate** (Declare war / Negotiate access) only if `can_assault` fails. Own land still marches (adjacent; Shift-click plans the next hex). Unit card **Attack** is the same path. Minimap hidden; card clips. Stacks show peeking plates + **1/3**; click the chip or **[ ]** cycles the unit card **and** the map NATO. F5 `can_assault` uses cheap `land_combat_power` (no CombatResolver). Freeze notes: [`SESSION_NOTES/2026-09-16_maginot_combat_learnings.md`](SESSION_NOTES/2026-09-16_maginot_combat_learnings.md).
