...
- **Panel close (playtest):** Close / Esc holds the inspector shut and **does not write the camera** (snapshot restore teleported Europe→Greenland). Close fires on press; pick-through / coarse-NA focus / clamp are blocked. Fills stay painted (never cull). A new hex pick reopens it.
- **Pan-from-drag (playtest):** left-drag pans after 8px slop; a click still picks. Release measures from the original press (and `_left_gesture_panned`) so a pan cannot also pick Lisboa. Chip / Ctrl / Shift clicks stay immediate. MMB and right-drag unchanged. Map click releases search LineEdit focus so G is not typed as `Berling`.
- **Home key (playtest):** Home / Shift+Home handled in `_input` so search/GUI focus cannot swallow recenter Europe / fit world.
- **End key (playtest):** frames Tokyo + Beiping `902487` at operational zoom, stamps both capital stars, and pins a visible **China** nation label (survives viewport cull / label rebuild).
