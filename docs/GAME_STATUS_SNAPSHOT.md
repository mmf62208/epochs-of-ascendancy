...
- **Pan-from-drag (playtest):** left-drag pans after 8px slop. Pick is skipped for **this left-button-down** once slop exceeds 8px (hex, sea, coarse, capital-star snap). Leftover `pressed=true` has Input down so it looks like a real click — `_begin(true)` must **not** reset slop (56b5479 physical-down reset picked Steinburg / East Asia coarse). Skip stays until Home/End/wheel/WASD or 8px of **mouse-up** motion (`_left_ready_for_still_click`), then a later still-click can pick (Berlin after Home). `_allow` must not clear slop. No time-box. No province-id special case. Every mouse writer gates on `_left_map_pick_blocked()`. Chip / Ctrl / Shift clicks stay immediate.
