...
- **Pan-from-drag (playtest):** left-drag pans after 8px slop. Pick is skipped for **this left-button-down** once slop exceeds 8px (`_left_gesture_dragged`). A duplicate `pressed=true` at the release hex is still THIS drag — `_begin_left_map_gesture(true)` must not start a slop-0 gesture while `_left_skip_next_pick` is set (Play: Orne / Sarthe). After this frame's input, `_allow_left_pan_skip_to_die` lets a later click `_begin(true)` with slop 0. No time-box. No idle-clear of dragged. No province-id special case. Every mouse writer gates on `_left_map_pick_blocked()`. Chip / Ctrl / Shift clicks stay immediate.
