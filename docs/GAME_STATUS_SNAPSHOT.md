...
- **L1 composition + combat losses (machine):** Designer mounts infantry with **motorcycle / truck / half-track** and optional tanks. Speed = **slowest remaining element** (foot infantry + tanks = walk; trucks mount infantry). Armor/defense feed attack vs defend power. Daily land tick writes **manpower and equipment for attacker and defender**. Product `unit_composition_combat_product` on `--quick`.
