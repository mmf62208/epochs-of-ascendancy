...
- **L1 G/WarLoop hang-class (playtest):** **G** key frame never BFS/preview_player_route (toast + deferred hop-capped land path, or click-to-show). Polyline via `highlight_supply_route_path`. **L-on** still no 3520 BFS. Shift+I stays toast-only. I defers flow-layer setup. Inspector **Close** (and Esc / unit-card Close) dismisses inspector + unit card + tooltip and `gui_release_focus` so keys/clicks return. `highlight_supply_corridor` still exists for tests.
