...
- **Panel close (playtest):** Close / Esc holds the inspector shut, snapshots the camera, and skips the same-frame hex pick so Europe does not jump to Greenland. Fills stay painted (never cull). A new hex pick reopens it.
