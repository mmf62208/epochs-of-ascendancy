...
- **L1 resolve hang-class (playtest):** F5 attacker-win is data-only (`_apply_attacker_win_capture_light`): silent owner flip, no second `execute_province_assault`, no FormationMovement, no owner signal storm. Deferred off the sim tick. Next day flush waits an extra frame after resolve. Encircle BFS skipped while interactive. Time keeps advancing after `open land battles resolved`.
...
- **L1 multi-day battle live (machine):** `start_land_battle` opens a front (empty defender still instant). TimeManager `_tick_open_land_battles` drains org 2–6 days; F5 attacker-win is data-only capture (no second resolve). Owner-change silent on light sim. `LandBattleBubbleLayer` + card Halt/Withdraw/Assign. Template power via `LandCombatPower` (armor ≠ infantry).
