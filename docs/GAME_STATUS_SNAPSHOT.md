...
| Player↔map interconnect (Mike Layer 2) | **IX-1 Road Spine** — GER Rhineland `710416` Bonn–`710417` Köln–`710418` Leverkusen; live Search **Köln/Cologne/Koln** + Go force-opens province inspector (Build Road Spine) — city-name + umlaut fold, soft pan, inspector z above Garrison; Alt/infra empty-terrain backup; Esc/Close restores; inspector Build Road Spine → first-session Mandate grant (GER 1936 day-0 Mandate 0) → `build_road_connection` + RoadLayer + cheaper `get_movement_cost`; F5 day-tick FIX (graphical Play skips full-board AI invest + day-7 calendar/quit autosave; harvest player-owned only; live-equiv gate past day +6) so softpipe 1× advances; see [`IX1_ROAD_SPINE.md`](IX1_ROAD_SPINE.md) · HOLD merge |
