...
**Date:** 2026-09-17 (L1 Maginot **click-order combat loop** + F5 **callee hang-class**: CombatResolver/composition/preview stub on light sim · RSS tripwire 2.5 GB · stacks cycle + per-unit orders · hover name+owner · names zoom toward capital · 2026-09-17 play catalogue)  
...
| **Machine** | Occupy-after-win headless: break keeps FRA, walk-in GER. JOINING roster. No inspector on command click. Catalogue: [`SESSION_NOTES/2026-09-17_maginot_play_catalogue.md`](SESSION_NOTES/2026-09-17_maginot_play_catalogue.md). Soft 30fps still FAIL honest. |
...
- **L1 click-order (playtest):** selected chip **right-click** / **Ctrl+click** enemy **starts the fight** (`start_land_battle`) when the assault is legal — no extra box. Compact **will-gate** (Declare war / Negotiate access) only if `can_assault` fails. Own land still marches (adjacent; Shift-click plans the next hex). Unit card **Attack** is the same path. Minimap hidden; card clips. Stacks show peeking plates + **1/3**; **left-click the chip** or **[ ]** cycles the stack (does not cancel orders). **Right-click the same hex** cancels that division's march/attack. F5 `can_assault` uses cheap `land_combat_power` (no CombatResolver). Freeze notes: [`SESSION_NOTES/2026-09-16_maginot_combat_learnings.md`](SESSION_NOTES/2026-09-16_maginot_combat_learnings.md).
