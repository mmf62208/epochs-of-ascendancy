...
- **Pan-from-drag (playtest):** left-drag pans after 8px slop. Pick is skipped for **this left-button-down** once slop exceeds 8px (hex, sea, coarse, capital-star snap). Leftover `pressed=true` has Input down — `_begin(true)` must not reset slop. Skip stays until Home/End/wheel/WASD (`_left_ready_for_still_click`). **Do not** arm on mouse-up motion (1680687 8px after `_end` armed leftover and picked Ille-et-Vilaine). `_left_map_pick_blocked()` is true whenever not ready, even if leftover wiped flags. Berlin after Home still picks. No time-box. No province-id special case. Chip / Ctrl / Shift clicks stay immediate.
