...
- **Stockpile → TOE (playtest):** unit card shows Fill% + stock rifles/trucks; daily reinforce toasts `Stockpile → TOE · N units · N eq`. First-session card promotes `Fill NN% · TOE …` as its own 16px SUCCESS/WARNING label above the clip fold (Speed/Armor/Men + last-3 combat log on tooltip). Product `unit_card_fill_toe_visibility_product` on `--quick`.
