...
- **Panel close (playtest):** Close / Esc locks the **live GIS camera** and reasserts it every frame (legacy `europe_center()` is Greenland/NA on world_accurate). Fight-sheet / unit-card Close go through the same dismiss. Same-click pick-through is guarded until the Close button is released. Fills stay painted (never cull). Home / End / WASD / wheel / a new map press unlock. A new hex pick reopens the inspector.
- **Pan-from-drag (playtest):** left-drag pans after 8px slop; `_left_pan_committed` is set when the camera actually moves and is **not** cleared on release — a drag that panned cannot also pick (Mid-Atlantic `#950181`). Click-without-slop still picks. Chip / Ctrl / Shift clicks stay immediate. MMB and right-drag unchanged. Map click releases search LineEdit focus so G is not typed as `Berling`.
