...
- **Beta playtest clock (machine):** `TimeManager.advance_living_playtest_days(5)` advances ≥5 calendar days, emits `game_day_advanced`, ticks open land battles with sync light capture, and records hang-safe AAR + capture news history (next-hex is first adjacent enemy; toast UI skipped headless; deferred map refresh still skipped on the compact clock). Headless quit skips calendar autosave (`_skip_quit_autosave`). Never `execute_province_assault` on that clock. Not beta-complete (human B0 still open; F5 1x 5–10d notes still human).
