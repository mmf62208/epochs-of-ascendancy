...
- **L1 G/WarLoop hang-class (playtest):** **G** key frame never BFS/preview_player_route (toast + deferred hop-capped land path, or click-to-show). Polyline via `highlight_supply_route_path`. **L-on** still no 3520 BFS. Shift+I stays toast-only. `highlight_supply_corridor` still exists for tests.
...
| **Unit counters (pins)** | OOB/NATO map chips · **culled at strategic** (capitals/fronts clickable) · operational+ full chips · **Shift+U** master toggle · nation plate + org/str bars · docked unit card · **pin-first pick when visible** (48px disk / floor 20 · gold chip · no inspector · `[` `]` stack) · `map_unit_counter_lod_product` + `unit_centric_pick_product` + `unit_counter_chrome_product` |
