...
1. **Main-loop combat and map campaigns**
   ...
   - [ ] Add simple AI battle initiation from formations, map state, and agent pressure.
...
2. **Active infrastructure investment loop**
   - ✅ `InfrastructureDevelopmentManager` skeleton, sabotage/repair, TimeManager tick, and InfoPanel invest entry exist.
   - [ ] Show project progress, ETA, owner/controller, and effect preview in InfoPanel and overlays.
   - [ ] Harden active project persistence, cancellation, completion, and migration edge cases.
   - [ ] Add basic AI investment priorities using supply chokepoints, factories, cities, and front pressure.
   - [ ] Verify NMM, agent, technology, engineer, population, sabotage, and settlement modifiers end-to-end.
3. **Save/load hardening for long sessions**
   - ✅ Broad persistence exists for time, tech, agents, supply, map, NMM, leaders, production, and infrastructure.
...
| Area | Next step |
|------|-----------|
| Combat | AI initiation, multi-day campaigns, live air/naval/weather/settlement effects |
| Leaders | Training bonuses in live battles, doctrine-gated paths, news feed panel |
| Technology | More 1918-1938 project content, diffusion UI, balance tests |
...
| AI | Basic invest/produce/attack logic using doctrines, pillars, and agents |

---

## Recently completed (June 2026)

- **Jun 15:** Top-bar player country selector; ESC closes topmost UI first; safer F9 load path; weather daily tick wiring; ScenarioLoader lookup fixes; draggable panel clamping; interactive time enabled by default with `EOA_FREEZE_TIME=1` smoke mode; missing Godot script UID added.
...
1. Combat battle loop on map: formations → AI/player orders → resolver → aftermath → owner/border/supply changes.
2. Infrastructure investment Phase A completion: player UX, AI projects, modifiers, save/load, overlay feedback.
3. Save/load hardening: active infra, formations, agent impacts, diffusion, scenario metadata, autosave UX.
