...
> **Completion program:** [`docs/MASTER_COMPLETION_PLAN.md`](docs/MASTER_COMPLETION_PLAN.md) — phased plan for **all** pillars (map, combat, designers, AI, multiplayer, content). Nothing is permanently deferred.  
> **Tier A playability:** [`docs/COMPLETION_PLAN.md`](docs/COMPLETION_PLAN.md) — Campaign Alpha primary strip (Phase 1).  
> **Playtest / decision cycle:** [`docs/PLAYTEST_AND_DECISION_GUIDE.md`](docs/PLAYTEST_AND_DECISION_GUIDE.md) · status assessment [`GAME_STATUS_ASSESSMENT.md`](GAME_STATUS_ASSESSMENT.md) · dual ground truth [`TODO.md`](TODO.md).  
> **Cursor:** open project with `cursor .` or `cursor-eoa`; use Master Plan §8 prompt packs.

## Combat / production / logistics residual (landed 2026-07)

- [x] **CP0–CP6** — design freeze + EquipmentFlow + stock/reinforce + munitions/drone + combat consume + map paint + deep combat/AI logistics duals  
- [x] **RF0–RF6** — non-instant reinforce · XP dilution · training policy · AI training · story plains  
- [x] **Map flow LOD + KEY_I glyph toggle** — dual `map_flow_lod_primary_live`  
- [x] **AI logistics in strategic AI daily apply** — dual `ai_logistics_day_primary_live`  
- [ ] **Playtest cycle (human)** — freeze new residual dual packages · use playtest guide · balance + visibility  

Catalogue labels: equipment_flow · reinforce logistics · combat production · map flow LOD · AI logistics day  
...
- Multiplayer — N3/N4 dual foothold landed; product multiplayer still open (see PLAYTEST guide)  
...
### Deferred key items (updated)
- Multiplayer — netcode residual foothold (N3/N4 duals); product still open  
...
- **~~Full Natural Earth province geometry for all 2665~~ LANDED** — NE 10m land ring snap complete; optional higher-res fjord mesh later  
...
- [x] **#5 Production auto · risk alert · day results flair + GIS×1343 dual** — next-90 live command  
...
- [x] **#5 Assault card · save slot list · multi-phase estimate · campaign strip + GIS×1343 dual** — next-80 execution  
...
- [x] **#5 Daily theater tick · command log · integrity gate + GIS×1343 dual held** — next-70 playability  
...
- [x] **#5 GIS×1343 + dual bar held** — map CI · next60 command pure gates  
...
- [x] **#5 GIS×1343 + dual bar held** — map CI · next50 ops pure gates  
...
- [x] **#5 GIS×1343 + dual bar held** — map CI · next40 campaign pure gates  
...
- [x] **#5 GIS×1343 + dual bar held** — map CI · next30 theater pure gates  
...
- [x] **#5 GIS×1343 + dual bar held** — map CI · next20 priority pure gates  
...
- [x] **#5 GIS×1343 + dual bar held** — map CI · next10 pure gates  
...
- [x] **#5 GIS×1343 + dual bar held** — map CI green  
...
- [x] **#5 GIS×1343 held** — littoral pool regression bar  

# Project State Summary — Epochs of Ascendancy

**Date:** 2026-07-12 catalogue · **Board/default refreshed 2026-08-12**  
**Role of this doc:** Historical day-wave planning contract (not the append-only dump in `docs/CURRENT_STATE.md`).  
**Live status / full-test director:** [`docs/GAME_STATUS_SNAPSHOT.md`](docs/GAME_STATUS_SNAPSHOT.md) · [`docs/GAME_DIRECTOR_PLAN.md`](docs/GAME_DIRECTOR_PLAN.md)  
**Play entry:** `scenes/TestScenario.tscn` · Godot **4.7.1+** · launcher `tools/run_godot.sh`  
**Default scenario:** **`world_accurate`** → `data/provinces_world_accurate` (**~3520** GIS hybrid)  
**Scaffold fallback:** `EOA_SCENARIO=world_full` → `data/provinces_world_full` (**2665**; IDs never renumbered)

---

## 1. What this game is

Grand strategy across **1918 / 1936 / 2026** starts: empire, industry, tech, agents, combat, and ascendancy. Target feel: **HOI4 + Terra Invicta + Supreme Ruler**, with unique **Hidden Hand / pillars** flavor. Engine: **Godot 4.x**, data-driven JSON, moddable architecture. Naval play, player agency, and strategic decisions stay non-negotiable.
...
| **5** | **Basing/sea/HH trail/agent + GIS×1343 dual** | Close next-100 | **LANDED** — **next-100 world-class** |
...
| **5** | **Production auto / risk / results flair + GIS×1343** | Close next-90 | **LANDED** — **next-90 live command** |
...
| **5** | **Card / save slots / multi-phase / campaign strip + GIS×1343** | Close next-80 | **LANDED** — **next-80 execution** |
...
| **5** | **Theater tick / command log / integrity + GIS×1343** | Regression + next-70 playability | **LANDED** — **next-70 playability** |
...
| **5** | **GIS×1343 dual held** | Regression + next60 pure gates | **LANDED** — **next-60 command** |
...
| **5** | **GIS×1343 dual held** | Regression + next50 ops pure gates | **LANDED** — **next-50 ops** |
...
| **5** | **GIS×1343 dual held** | Regression + next40 campaign pure gates | **LANDED** — **next-40 campaign** |
...
| **5** | **GIS×1343 dual held** | Regression + next30 theater pure gates | **LANDED** — **next-30 theater** |
...
| **5** | **GIS×1343 dual held** | Regression + next20 priority pure gates | **LANDED** — **next-20 priority** |
...
| **5** | **GIS×1343 dual held** | Regression + next10 pure gates | **LANDED** — **next-10 depth** |

## Top 5 priorities — next 48 hours (WEEK-4 POLISH)

| # | Priority | Outcome | Status |
|---|----------|---------|--------|
| **1** | **GPU pan/zoom day** | Soft load/icon/cull advice; no hard gate | **LANDED** — **gpu pan/zoom day** |
| **2** | **Tooltip/SFX flair strip** | Select/invest/assault flair contracts strip | **LANDED** — **tooltip/sfx flair strip** |
| **3** | **Docs retrospective** | Master docs mark Weeks 1–4 complete | **LANDED** — **week-4 polish** |
| **4** | **Week-3 naval/HH held** | Agenda/fleet/sealane | **LANDED** |
| **5** | **GIS×1343 dual held** | Regression bar | **LANDED** |

## Top 5 priorities — next 48 hours (WEEK-3 NAVAL + HH DEPTH)

| # | Priority | Outcome | Status |
|---|----------|---------|--------|
| **1** | **HH agenda screen day** | Sections + one-click apply queue beyond pilots | **LANDED** — **hh agenda screen day** |
| **2** | **Fleet autonomy day** | Multi-province autonomy apply queue | **LANDED** — **fleet autonomy day** |
| **3** | **Sealane contest visual** | Choke glyph tint by sealane contest visual | **LANDED** — **sealane contest visual** |
| **4** | **Week-2 polish held** | Apply audit · save flair | **LANDED** |
| **5** | **GIS×1343 held** | Littoral pool bar | **LANDED** |

## Top 5 priorities — next 48 hours (WEEK-2 CORE SYSTEMS)

| # | Priority | Outcome | Status |
|---|----------|---------|--------|
| **1** | **Day-package apply audit** | All panel day routes live in apply_order_panel_action (dead=0) | **LANDED** — **day apply audit** |
| **2** | **Save-slot browser flair** | Empty/occupied flair markers on slot browser | **LANDED** — **save-slot flair** |
| **3** | **Infra/site consistency skim** | Sites vs infra vs projects consistency chip | **LANDED** — **infra/site consistency** |
| **4** | **GIS×1343 + inspector budget held** | Littoral pool + chip budget regression | **LANDED** |
| **5** | **Prior UX Top 5 held** | Panel sections · dual green | **LANDED** |

## Top 5 priorities — next 48 hours (WEEK-1 MAP FINISHERS)

*Same Top 5 as `Next_30_Days_Roadmap.md` and `TODO.md`. Prior UX Top 5 complete — do not re-open.*

| # | Priority | Outcome | Status |
|---|----------|---------|--------|
| **1** | **GIS littoral pool fill** | Stamp **≥753** coastal+littoral (full pool) id-stable, triangles=0 | **LANDED** — **GIS×1343** · fixture 753 · pure GIS tests |
| **2** | **Inspector product-depth chip budget** | Cap product-depth day chips (max 8) with always-on naval/HH/campaign | **LANDED** — **inspector product-depth** budget via `budget_product_depth_chips` |
| **3** | **Resource/damage operational skim** | Operational zoom resource+damage readability | **LANDED** — **resource/damage skim** chip |
| **4** | **Sealane contest skim** | Zone · choke · escort · contest % one-liner | **LANDED** — **sealane contest skim** chip |
| **5** | **Prior UX Top 5 held** | Panel sections · naval skim · HH path · medium-horizon equip · dual green | **LANDED** — regression bar held |

**Evidence (this delivery):** `/tmp/grok-goal-21e7aa1dbab9/implementer/` — dual headless · map CI · GIS expand logs · `EVIDENCE.md`

---

## 2. Architecture snapshot (verified live)

| Area | Path | Status |
|------|------|--------|
| Map runtime | `scripts/map/` (~35 GD scripts) | Strong; **MapRenderer** is the largest surface |
| Autoloads / managers | GameData, MapManager, TimeManager, InfraDev, SpecialSites, SaveLoad, Production, Supply, Leaders, Agents, Tech, Battle, Weather, Trade, … | Live; GameData monolith risk grows with day packages |
| Primary map data | `data/provinces_world_accurate/` | **~8761** GIS hybrid (default F5); terrain/resources/economy/city/ownership/regions/chokepoints/sea zones |
| Scaffold dual | `data/provinces_world_full/` | **2665** CI dual / fallback; IDs never renumbered |
| Europe baselines | `provinces_full_europe` **460** · `provinces_phase1_test` **507** | Named densified; not default F5 |
| Grand theater | `provinces_grand_theater` ~1133 | Multi-theater underlay path |
| Legacy | `data/provinces/` 840 base / 100 geom | Do not treat as play theater |
| Pipeline | `tools/map_generation/` | expand / densify / organicize / names / ownership / regions / city / choke / sea / **GIS coastal+littoral pilot** / CI |
| Scenes | `TestScenario.tscn`, `WorldMap.tscn`, `scenes/ui/*` | F5 harness + map |
| Design docs | `docs/MAP_*`, `DESIGN_Infrastructure*`, `HIDDEN_HAND_*`, `GIS_COASTLINE_*` | Rich; some long-form docs lag live world_full |

### Core managers (health)

| Manager | Role | Health |
|---------|------|--------|
| **MapManager** | Province authority, pick grid, ownership, roads/rails, chokepoints, sea zones, day packages | Solid API; day-helper surface growing |
| **MapRenderer** | Polys, modes, underlays, combat staging, inspector | Feature-rich; polish/perf still open |
| **InfrastructureDevelopmentManager** | Projects, AI invest, sabo | Live; era-band density landed |
| **SpecialSiteManager** | ~26 defs + placement | Icons/effect chips live |
| **SaveLoadManager** | Full slot + map/production/leaders/design/factories | Hardened; post-load conquest path tested |
| **ProductionManager** | Lines, stockpiles, equip, reinforce, combat stats | Equip loop proven; **OOB @ 20d majors_grew 7/7** |
| **BattleManager** | Assault, capture, chain/flank, station/displace | Campaign continuity strong |
| **DebugOverlay (F10)** | Large surface | Player Map vs Dev Harness split done |
| **GameData** | Events/policy/HH/order-panel day routes | HH three map classes + agenda trail; product-depth day stack |

---

## 3. What is strong today

1. **Full world playable board** — ~8761 accurate default + 2665 scaffold dual, ownership 1936, factories/OOB stations/leaders (leaders file for world_accurate still D1 gap), map CI green, dual headless markers green.
2. **Map systems architecture** — layered JSON, MapPickGrid, zoom LOD, viewport cull all tiers ≥2200, political labels, infrastructure/weather/agent/conflict overlays, naval mapmode, **56** chokepoints, **19** sea-zone theaters with control + supply/trade modifiers.
3. **Geometry / regions / names (map-star pass)** — verts min **16** / median **18** / **0 triangles**; regions max share **11.1%**; unique names 2665; cities **900** (far=0).
4. **GIS coastline pilot expanded** — **240 → 564 → 720 → 753** id-stable coastal+littoral rings stamped (`meta.gis_pilot`); full littoral pool filled; **not** full Natural Earth photoreal.
5. **Combat ↔ industry continuity** — equip, reinforce, capture→station/displace→chain, factory seize, design grant, post-load playability + slot save/load; OOB **7/7 majors @ 20d**.
6. **Product-depth day packages** — logistics, war economy, theater, joint command, strategic continuity, force readiness, industry surge, joint campaign, weather crisis, agent campaign, combat campaign, fleet campaign stacked on multi-province daily theater tick.
7. **Hidden Hand MVP+** — three map classes, multi-month agenda trail, panel/screen/action-pick pilots, counterplay path.

---

## 4. Issue inventory (ruthless, map-first)

### P0 — Map is credible but not yet HOI4-star

| Issue | Evidence (2026-07-12) | Impact |
|-------|----------------------|--------|
| **GIS / NE province rings** | **2665/2665** `gis_ne_full` + `gis_pilot` (NE 10m land snap) · triangles=0 · min verts ≥20 | **COMPLETE** for play geometry; optional NE 50m/fjord micro-detail later |
| **Gazetteer approximate** | Residual robotic Sector/Basin cleared; soft place-names still approximate | Player immersion mid |
| **Naval ops model thin** | 56 chokes + 19 sea zones + basing + fleet campaign day; no full fleet AI | Naval strategy readable, not deep AI |
| **Data-dir dualism** | world_full vs grand_theater vs full_europe vs legacy 840 | Operator confusion mitigated by `resolve_play_data_dir` |

### P1 — Core loop honesty

| Issue | Evidence | Impact |
|-------|----------|--------|
| **Medium-tank OOB** | Product surface + progress evidence print; infantry @ 20d; mediums multi-month | Multi-month equip honesty **in progress** (product + progress evidence) |
| **Inspector density** | Top-line + many chips; collapse incomplete | Cognitive load high |
| **Order panel sprawl** | Dozens of day-apply buttons | Power-user strong; new-player weak |
| **Slot UI polish** | APIs round-trip; browser UX thin | Save/load power-user only |

### P2 — Hidden Hand

- Three map-visible classes live  
- Trail + panel + screen layout + action-pick pilots live  
- Missing: full multi-faction agenda **screen**, black-market depth, campaign-length counterplay  

### P3 — Performance

- All-tier cull + resource icon budget **180** + mesh batch @ ≥2200 shipped  
- Dual headless green  
- Graphical pan/zoom on mid hardware still worth a hard profile (soft deferred)  
- MapRenderer size = maintainability + frame-time risk  

### P4 — World-class touches

- Resource icons + damage visuals live; density/readability can improve  
- Sound/tooltip flair partial  
- Majors #1–#6 product surfaces **shipped** (combat · fleet · medium OOB · save browser · HH multi-month · agent campaign) · multiplayer/deep AI/full designers still later  

---

## 5. Pipeline truth

| Capability | Script / lib |
|------------|----------------|
| World expand / hotspots | `expand_world_provinces.py`, `densify_world_hotspots.py` |
| Geometry densify / organicize | `densify_province_geometry.py`, `organicize_seed_geometry.py` |
| Names / ownership / regions | polish/assign/rebuild/relabel scripts |
| Cities / chokes / sea | seed_world_city_layer, expand_world_naval_chokepoints, build_sea_zone_theaters |
| **GIS coastal + littoral pilot** | `ingest_gis_coastlines.py` + `gis_coastline_ingest.py` (`--include-littoral`, `--pilot-limit`, guarded `--write --pilot`) |
| CI | `tools/run_map_ci.sh data/provinces_world_full` |

Largest remaining pipeline gap: **~~full-world Natural Earth coastline rewrite~~ LANDED** (2665/2665 NE full align). Further: fjord mesh detail / higher-res NE 50m optional.

---

## 6. Scenario / load path

1. Main scene → `TestScenario` → ScenarioLoader  
2. Default `world_accurate` → `provinces_world_accurate` (~8761); scaffold `world_full` via env  

3. Industry bootstrap + leader OOB + starting stockpiles + optional OOB evidence ticks (`EOA_OOB_EVIDENCE_DAYS`)  
4. MapRenderer world underlay + province mesh + overlays  

**Never** use `EOA_HEADLESS_EVIDENCE=1` for dual (OOM). Prefer `EOA_OOB_EVIDENCE_DAYS=20` + kill-after-markers.

---

## 7. Campaign continuity bar (keep green)

- Map CI exit **0** on world_full  
- Dual headless: **2665** · SCRIPT/Parse/Compile ERROR **0** · Industry bootstrap · Formation equip · Daily production **7/7 @ 20d** when OOB days set  
- Pure GIS gates: stamped **2665/2665** `gis_ne_full`, triangles=0, min verts ≥16, id count 2665  

---

## 8. Product-depth day stack (live multi-tick)

… weather_crisis → agent_campaign → combat_campaign → **fleet_campaign** (and earlier logistics / war_economy / theater / joint / force / industry packages).

Each package: pure lib → MapPolishFormatters → MapManager → GameData apply + router + day tick → OrderCommandPanel → ProvinceInsight chips → pure wiring tests → map CI hook.

---

## 9. Non-goals this phase

- ~~Full Natural Earth province geometry for all 2665~~ **LANDED** (NE 10m full stamp)  
- Multiplayer product · full HOI4 3D designers · full HOI4-parity combat/AI in one pass · playtest cycle first  
- Replacing Godot version or rewriting MapRenderer from scratch  
- Re-opening completed map-star Top 5 as unfinished work  

---

## 10. Evidence pointers (this delivery)

- Scratch: `/tmp/grok-goal-21e7aa1dbab9/implementer/`  
- GIS expand logs · map CI · dual headless · `EVIDENCE.md` under that path after verification run  

Companion roadmap: `Next_30_Days_Roadmap.md` · checklist: `TODO.md`
...
**UX + inspector labels (CI):** order panel sections · naval campaign skim · hh player path · medium-horizon equip · inspector product-depth · resource/damage skim · sealane contest skim · GIS×1343

**Week-2 polish labels (CI):** day apply audit · save-slot flair · infra/site consistency · GIS×1343

**Week-3 depth labels (CI):** hh agenda screen day · fleet autonomy day · sealane contest visual · GIS×1343

**Week-4 polish labels (CI):** gpu pan/zoom day · tooltip/sfx flair strip · week-4 polish · GIS×1343
